...
| :----------------------------------------------------------- | :---------------------------------------------------------------------------------------------------------------------------------------- | :------------------------------------------------------------------------------ | :--------------------------------- | :--------- |
| No Node `config` print command exists                        | `wpdev-build config` emits Go sorted-key JSON with `SetEscapeHTML(false)` (R-007c)                                                        | `TestRun_ConfigJSONNoHTMLEscape` + U01 oracle `<`/U+2028 vectors                | independent reviewer (antigravity) | 2026-09-22 |
| `getRootPath()` is module-anchored (`path.js`)               | CLI discovery walks from cwd/`StartDir`; library contract is `config.ReadFile` (R-008g)                                                   | `TestRead_DiscoverySkipsDirectoryNamedWpdevJSON`, `TestReadFile_DirectoryFails` | independent reviewer (antigravity) | 2026-09-22 |
| `covercheck` grouping is posix-only (no such tool in source) | drive letter retained in package key (`C:/...`), locked by `TestEvaluate_WindowsDrivePath` + `TestEvaluate_WindowsBackslashPath` (R-008e) | `covercheck_test.go:107-140`                                                    | independent reviewer (antigravity) | 2026-09-22 |
