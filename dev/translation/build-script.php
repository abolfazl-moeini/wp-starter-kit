...
wpdev_color_log('Building script JSON translation files...');
...
foreach (wpdev_list_components() as $component) {
    ...
    if (!is_file($map_file)) {
        wpdev_color_log("    map missing — run translation:generate first", 'w');
        continue;
    }
    if (!is_file($asset_file)) {
        wpdev_color_log("    bundle asset missing — run npm run build first", 'w');
        continue;
    }
    ...
    if (!$r['ok']) {
        wpdev_color_log("    wp i18n make-json: SKIP (" . trim($r['stderr']) . ")", 'w');
        continue;
    }
    wpdev_color_log("    wp i18n make-json: OK", 'g');
}
