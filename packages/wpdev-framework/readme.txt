...
WPDev Framework — modular WordPress admin framework with optional WaaS Examples.

== Description ==

WPDev Framework

Modular admin framework for WordPress. Optional WaaS Examples (`examples/*`) provide the full Website-as-a-Service demo suite with existing `wpdev-*` admin pages.

== Installation ==

1. Upload 'wpdev' to the '/wp-content/plugins/' directory
...
= 2.5.0 =
Major modularization release. Staging strongly recommended. After updating, copy `wp-content/plugins/wpdev/sunrise.php` to `wp-content/sunrise.php` on multisite installs with SUNRISE enabled. Run `composer release:gate` or `composer regression:docker` from the plugin directory to verify.

We recommend running this in a staging environment before updating your production environment.

== Changelog ==

Version [2.5.0] - Modularization release

- Changed: Canonical plugin code lives under `modules/`; legacy `inc/**/*.php` shims removed
- Changed: Multisite `sunrise.php` auto-updates to `wp-content/` when `WPDEV_SUNRISE_VERSION` is below 2.5.0 (paths use `modules/wizard/class-sunrise.php`)
- Fixed: Bootstrap loads main `WPDev` class from `modules/core` before `plugins_loaded`
- Improved: Automated regression and audit scripts (`composer release:gate`, `composer regression:docker`)
...
- Fixed: Ensure that the 'wpdev_original_cart' metadata is consistently set in payment during checkout process
...
- Improved: Change the WPDevFramework\Helpers\Sender::email_sender() calls to use as a static method only
...
- Improved: Removed unnecessary params in class WPDevFramework\Compat\Multiple_Accounts_Compat
...
- Added: Filter `wpdev_bulk_action_function_prefix` on `process_bulk_action()` method
...
- Added: A filter called "WPDev_my_sites_show_add_new" to allow developers to hide the Add New Site button.
- Added: Allowed the use of the wpdev_restricted_content shortcode with checkout form current products.
...
- Added: An option to set a log level to write WPDev logs (defaults to the current PHP report level).
...
- Changed: Allowed the use of the product_id variable in place of plan_id in the wpdev_restricted_content shortcode.
...
- Fixed: The legacy wpdev_restricted_content shortcode to correctly work with wpdev v2.
...
- Fixed: Switched to the main site in the wpdev_get_membership_customers function.
- Fixed: Disabled WP Core application password on WPDev Routes so we can use our API keys.
...
- Added: the filter "wpdev_checkout_add_field_{field_type}" to allow developers to change form fields in the checkout form
- Added: an option called "checkout_page" to set a page ID to redirect to `wpdev_templates_list` shortcode
...
- Changed: Filter WPDev admin bar menu items by capabilities
...
- Fixed: Filter WPDev checkout forms menu by `wpdev_read_memberships` capability
...
- Fixed: Redirect to the checkout page after selecting a site template on a `wpdev_templates_list` shortcode
...
- Added: the `wpdev_add_product_setup_fee_line_item` filter to the cart Setup Fee line item
...
- Added: `wpdev_after_switch_template` action (present in v1)
...
- Added: the WPDev plugin version as a const on the class `\WPDev`, allowing for access without the need for an instance - which might not yet exist
...
- Added: generation of a new .pot file containing countries, states, and cities using the new text-domain wpdev-locations
...
- Added: filter `wpdev_list_row_actions` to row actions on Admin List Pages
...
- Changed: Deprecated the use of the property `$version` on the class `\WPDev` in favor of a class const
...
- Fixed: Fixed the URL returned to register and other WPDev pages
...
- Fixed: Allow `wpdev_get_*` functions to be used as form field attributes callback
...
* Fix: Fix the use of wpdev_append_preview filter on plugins compatibility file;
...
* Added: Option in WPDev sites settings to allow the selection of a page on main site to create a new site;
* Added: Option in WPDev membership settings to allow the selection of a page on main site to update membership;
* Added: Optional constant wpdev_GATEWAY_LISTENER_URL to allow us to change the URL to use on gateway webhooks (usefull on dev environments);
...
* Fix: Paypal gateway with trial period in membership;
* Fix: MRR calculated value on wpdev dashboard for yearly memberships;
...
* Added: Security mode to deactivate all plugins except WPDev and mu-plugins at once and reactivate after disable;
...
* Added: wpdev_return_url filter, allowing custom redirections after checkout process;
...
* Added: Event to send email when a new payment is created via cron in WPDev (trial without payment method and manual payments for now);
...
* Added: Stripe gateways payment line items on WPDev payment created on renewals;
* Added: wpdev_setup_step_done_name filter on setup wizard;
...
* Added: wpdev_pre_save_settings filter to allow data change before save settings;
...
* Improvement: Improves the addition of billing address fields by allowing the removal of fields through the “wpdev_billing_address_fields” filter and avoiding error in the use of this filter;
* Improvement: Checks if payment status is completed when building the cart to prevent error with wpdev defining the cart as “retry”;
...
* Fix: Stripe webhook not being processed by wpdev;
...
* Fix: Value "enable_custom_login_page" added with value 1 on wpdev install process to avoid errors on wpdev setup page;
...
* Added: Created the wpdev_before_form_submitted javascript filter to allow bypass the checkout form before submit;
* Added: Sunrise.php data on WPDev system info page;
...
* Other
  ...
  * Fixed: Makes sure the auto-submittable script is only added after wpdev-checkout was loaded;
  ...
  * Added: Created the wpdev_bypass_unset_current_user filter to allow developers to bypass the user unset on multiple account feature;
  ...
  * Improvement: sunrise.php install step on WPDev Install Wizard;
  * Improvement: better define of SUNRISE constant on wp-config.php on WPDev Install Wizard;
  ...
  * Improvement: Adds a sunrise meta file to control whether or not we need to run wpdev tasks when wpdev is disabled or not present;
...
* Internal
  ...
  * Internal: Adds the Query Monitor panels to help debug issues while developing WPDev core;
  * Internal: Adds the development sandbox toolkit that allows developers to run and listen to particular events in a isolated context. Useful for timing how long a given hook takes to run, or to trigger build tasks that rely on a real WordPress installation running wpdev to work;
...
* Added: Workflow to automatically generate the [list of actions](https://github.com/next-press/wpdev-hooks-and-filters/blob/main/wpdev.md) and filters for the plugin;
...
* Fixed: Incompatibility between Blocksy customizer and mapped domains on WPDev;
* Fixed: Incompatibility between Brizy and mapped domains on WPDev;
...
* Fixed: Conflict with Fluent Forms - where wpdev broke the FF form edit UI;
...
* Added: Hook wpdev_checkout_after_process_order added - required by the new AffiliateWP Integration;
* Added: Filters for class-current - wpdev_current_site_get_manage_url, wpdev_current_set_site, wpdev_current_set_customer. Useful for integrations and later front-end management functionality;
...
* Added: Filter to change the /template/ portion of the pre-selected site template URL - "wpdev_template_selection_rewrite_rule_slug";
...
* Improvement: Added a "cols" parameter to the wpdev_templates_list shortcode - with a default value of 3;
...
* Fixed: Shortcode wpdev_pricing_table buttons now correctly select plans on the checkout form;
...
* Improvement: Added the skip_validation option to the wpdev_create_payment function - which is required for the migrator to work properly;
* Improvement: Adds dumb Mercator file and a Mercator load statement to the v2 sunrise file to increase compatibility with WPDev after a rollback is performed;
...
* Fixed: Removed the 100 site template limitation on the [wpdev_templates_list] shortcode and the Template Selection field;
* Fixed: Selecting a template from the [wpdev_templates_list] starts the registration with the template pre-selected correctly;
...
* Fixed: Added support for Elementor's render_widget ajax calls to prevent errors when saving pages that contain WPDev elements;
...
* Improvement: Wraps "wpdev_core_update" on a try/catch statement;
...
* Fixed: Filter "wpdev_domain_has_correct_dns" returning the wrong base value;
...
* Fixed: Prevents wpdev elements from breaking Divi - still needs work to make sure element previews display correctly inside the visual editor;
...
* Improvement: Load block editor fields for WPDev blocks with default values pre-loaded;
...
* Improvement: Add a warning when the sunrise.php is still being loaded, even when WPDev is no longer active;
...
* Fixed: Only register WPDev blocks and shortcodes on sites that are not customer-owned;
...
* Fixed: WPDev and its add-ons not appearing on the main site's plugins list;

Version 2.0.0 - Released on 2021-11-21 (WPDev's 5-year anniversary)
