...
abstract class Wizard_Admin_Page extends Base_Admin_Page {
	...
	public function register_widgets() {
		...
		if (wpdev_get_isset($this->current_section, 'separator')) {

			return;

		} // end if;

		add_meta_box('wpdev-wizard-body', wpdev_get_isset($this->current_section, 'title', __('Section', 'wpdev')), array($this, 'output_default_widget_body'), $screen->id, 'normal', null);
	...
	public function output() {
		...
		$template = class_exists( 'WPDev\\Modules\\AdminPageBuilder\\Page_Template_Registry' )
			? \WPDevFramework\Modules\AdminPageBuilder\Page_Template_Registry::resolve( 'wizard', 'base/wizard' )
			: 'base/wizard';
		...
		wpdev_get_template( $template, array(
			...
			'classes'              => 'wpdev-w-full wpdev-mx-auto sm:wpdev-w-11/12 xl:wpdev-w-8/12 wpdev-mt-8 sm:wpdev-max-w-screen-lg',
	...
	public function get_current_section() {
		...
		$sections = array_filter($sections, fn($item) => wpdev_get_isset($item, 'addon') === false);
	...
	public function default_view() {
		...
		$section = wp_parse_args($this->current_section, array(
			...
			'content'     => '',
			'fields'      => array(),
			'next_label'  => __('Continue &rarr;', 'wpdev'),
			'back_label'  => __('&larr; Go Back', 'wpdev'),
			'skip_label'  => __('Skip this Step', 'wpdev'),
			'back'        => false,
			'skip'        => false,
			'next'        => true,
		));
		...
		if (!empty($section['fields'])) {
			...
			$form = new \WPDevFramework\UI\Form($this->get_current_section(), $section['fields'], array(
				'views'                 => 'admin-pages/fields',
				'classes'               => 'wpdev-widget-list wpdev-striped wpdev-m-0 wpdev-mt-2 wpdev--mb-6 wpdev--mx-6',
				'field_wrapper_classes' => 'wpdev-w-full wpdev-box-border wpdev-items-center wpdev-flex wpdev-justify-between wpdev-px-6 wpdev-py-4 wpdev-m-0 wpdev-border-t wpdev-border-l-0 wpdev-border-r-0 wpdev-border-b-0 wpdev-border-gray-300 wpdev-border-solid',
			));
		...
		wpdev_get_template('wizards/setup/default', array_merge($section, array(
			'page' => $this,
		)));
	...
	public function render_submit_box() {
		...
		wpdev_get_template('base/wizard/submit-box', array(
			'screen' => get_current_screen(),
			'page'   => $this,
			'labels' => $this->get_labels(),
		));
