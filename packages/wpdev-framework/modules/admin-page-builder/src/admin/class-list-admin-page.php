...
abstract class List_Admin_Page extends Base_Admin_Page {
	...
	public function get_labels() {
		...
		return array(
			'deleted_message' => __('Object removed successfully.', 'wpdev'),
			'search_label'    => __('Search Object', 'wpdev'),
		);
	...
	public function output() {
		...
		$template = class_exists( 'WPDev\\Modules\\AdminPageBuilder\\Page_Template_Registry' )
			? \WPDevFramework\Modules\AdminPageBuilder\Page_Template_Registry::resolve( 'list', 'base/list' )
			: 'base/list';

		wpdev_get_template( $template, array(
			'page'    => $this,
			'table'   => $this->get_table(),
			'classes' => $this->table->get_filters() ? 'wpdev-advanced-filters' : 'wpdev-no-advanced-filters',
		));
	...
	public function screen_options() {
		...
		if ( function_exists( 'wpdev_services' ) ) {
			$service = wpdev_services( 'screen_options' );

			if ( $service && method_exists( $service, 'register_per_page' ) ) {
				$service->register_per_page( $option, $label );
				return;
			}
		}
	...
	/**
	 * Dumb function. Child classes need to implement this to set the table that WPDev will use
	 *
	 * @since 1.8.2
	 ...
	 *
	 * @since 2.0.0
	 * @return \WPDevFramework\List_Tables\Base_List_Table
	 */
