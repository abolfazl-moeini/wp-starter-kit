...
class Template_Previewer {
	...
	public function init() {
		...
		add_action('wpdev_load', array($this, 'hooks'));
	...
	public function hooks() {
		...
		if ($this->is_preview()) {
			...
			add_filter('wpdev_is_jumper_enabled', '__return_false');

			add_filter('wpdev_is_toolbox_enabled', '__return_false');
		...
		if ($this->is_template_previewer()) {
			 ...
			 */
			do_action('wpdev_template_previewer', $this);
	...
	public function register_scripts() {
		...
		$bg_color         = wpdev_color($settings['bg_color']);
		$button_bg_color  = wpdev_color($settings['button_bg_color']);
		$button_bg_darker = wpdev_color($button_bg_color->darken(4));

		wp_register_script('wpdev-template-previewer', wpdev_get_module_asset_url('admin-widget-builder', 'template-previewer.js', 'js'), array(), wpdev_get_version());

		wp_localize_script('wpdev-template-previewer', 'wpdev_template_previewer', array(
			'domain'           => str_replace('www.', '', (string) $current_site->domain),
			'current_template' => wpdev_request($this->get_preview_parameter(), false),
			'current_url'      => wpdev_get_current_url(),
			'query_parameter'  => $this->get_preview_parameter(),
		));

		wp_enqueue_script('wpdev-template-previewer');

		wp_enqueue_style('wpdev-template-previewer', wpdev_get_module_asset_url('admin-widget-builder', 'template-previewer.css', 'css'), array(), wpdev_get_version());

		wp_add_inline_style('wpdev-template-previewer', wpdev_get_template_contents('dynamic-styles/template-previewer', array(
			'bg_color'        => $bg_color,
	...
	public function append_preview_parameter($url, $path, $orig_scheme, $blog_id) {
		...
		if (apply_filters('wpdev_append_preview_parameter', true, $this) === false) {

			return $url;

		} // end if;

		return add_query_arg('wpdev-preview', 1, $url);
	...
	public function template_previewer() {
		...
		$template_value = wpdev_request($this->get_preview_parameter(), false);

		$selected_template = wpdev_get_site($template_value);

		/**
		 * Check if this is a site template
		 */
		if ( $selected_template->get_type() !== Site_Type::SITE_TEMPLATE ) {

			if ( wpdev_request( 'customizer' ) ) {

				$can_customize = is_user_logged_in() && (
					( function_exists( 'wpdev_current_user_can_admin' ) && wpdev_current_user_can_admin( 'wpdev_edit_sites' ) )
					|| current_user_can( 'manage_network' )
				);

				if ( ! $can_customize ) {
					wp_die( esc_html__( 'You do not have permission to customize the template previewer.', 'wpdev' ) );
				}

			} else {

				wp_die( esc_html__( 'This template is not available', 'wpdev' ) );
		...
		$products_ids = isset($_COOKIE['wpdev_selected_products']) ? explode( ',', (string) $_COOKIE['wpdev_selected_products']) : array();

		$products = array();

		if ( function_exists( 'wpdev_get_product' ) ) {
			$products = array_map( 'wpdev_get_product', $products_ids );
		...
		if (!empty($products)) {
			...
			if ($limits->site_templates->get_mode() !== 'default') {
				...
				$render_parameters['templates'] = array_map('wpdev_get_site', $site_ids);
				...
				if (!in_array($selected_template->get_id(), $site_ids, true)) {
					...
					$redirect_to = wpdev_get_current_url();
		...
		if (!isset($render_parameters['templates'])) {
			...
			$render_parameters['templates'] = wpdev_get_site_templates();
		...
		wpdev_get_template('ui/template-previewer', $render_parameters);
	...
	public function get_preview_parameter() {
		...
		return apply_filters('wpdev_get_template_preview_slug', $slug);
	...
	public function is_preview() {
		...
		return !empty(wpdev_request('wpdev-preview'));
...
//		$GLOBALS['pagenow'] = '';
		...
		$default_settings = array(
			...
			'button_text'                 => __('Use this Template', 'wpdev'),
		...
		$saved_settings = wpdev_get_option(Template_Previewer::KEY, array());
		...
		$parsed_args['display_responsive_controls'] = wpdev_string_to_bool($parsed_args['display_responsive_controls']);
		$parsed_args['use_custom_logo']             = wpdev_string_to_bool($parsed_args['use_custom_logo']);
	...
	public function get_setting($setting, $default = false) {
		...
		return wpdev_get_isset($this->get_settings(), $setting, $default);
	...
	public function save_settings($settings_to_save) {
		...
		foreach ($settings as $setting => $value) {
			...
			if ($setting === 'logo_url') {
				...
				$settings['logo_url'] = wpdev_get_network_logo();

				continue;

			} // end if;

			$settings[$setting] = wpdev_get_isset($settings_to_save, $setting, false);

		} // end foreach;

		return wpdev_save_option(Template_Previewer::KEY, $settings);
