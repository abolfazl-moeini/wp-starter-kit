...
abstract class Base_Element {
	 ...
	 *
	 * This is used to construct shortcodes by prefixing the id with 'wpdev_'
	 * e.g. an id checkout becomes the shortcode 'wpdev_checkout' and
	 * to generate the Gutenberg block by prefixing it with 'wpdev/'
	 * e.g. checkout would become the block 'wpdev/checkout'.
	 *
	 * @since 2.0.0
	 * @var string
	 */
	 ...
	 * e.g. return __('My Element', 'wpdev').
	 *
	 * @since 2.0.0
	 * @return string
	 */
	 ...
	 * You should return a string with the localized title.
	 * e.g. return __('Adds a checkout form to the page', 'wpdev').
	 *
	 * @since 2.0.0
	 * @return string
	 */
	 ...
	 * our fields interface (@see modules/field-builder/src/field/class-field.php).
	 *
	 * You can create new Gutenberg panels by adding fields
	 * with the type 'header'. See the Checkout Elements for reference.
	 *
	 * @see examples/checkout/src/ui/class-checkout-element.php
	 *
	...
	public function init() {
		...
		add_action("wpdev_{$this->id}_scripts", array($this, 'register_default_scripts'));

		add_action("wpdev_{$this->id}_scripts", array($this, 'register_scripts'));
		...
		add_action('wpdev_element_preview', array($this, 'setup_preview'));

		do_action('wpdev_element_loaded', $this);
	...
	public function setup_for_block_editor($short_circuit, $block) {
		 ...
		 * @return array
		 */
		$blocks_to_check = apply_filters('wpdev_element_block_types_to_check', array(
			'core/shortcode',
			'core/paragraph',
		));
	...
	protected static function search_in_metaboxes($element_id) {
		...
		$contains_metaboxes = wpdev_get_isset($wp_meta_boxes, $screen->id) || wpdev_get_isset($wp_meta_boxes, $pagenow);
	...
	public function setup_for_admin() {
		...
		$element_id = "wpdev-{$this->id}-element";
	...
	protected function contains_current_element($content, $post = null) {
		 ...
		 */
		if ($post && get_post_meta($post->ID, '_wpdev_force_elements_loading', true)) {
		 ...
		 */
		return apply_filters('wpdev_contains_element', $contains_element, $content, $this, $post);
	...
	protected function maybe_extract_arguments($content, $type = 'shortcode') {
		 ...
		 * @return false|array
		 */
		return apply_filters('wpdev_element_maybe_extract_arguments', false, $content, $type, $this);
	...
	public function enqueue_element_scripts() {
		...
		$should_enqueue_scripts = apply_filters('wpdev_element_should_enqueue_scripts', false, $post, $this->get_shortcode_id());
		...
		if ($should_enqueue_scripts || $this->contains_current_element($post->post_content, $post)) {
			 ...
			 * @since 2.0.0
			 */
			do_action("wpdev_{$this->id}_scripts", $post, $this);
	...
	public function get_pre_loaded_attribute($name, $default = false) {
		...
		return wpdev_get_isset($this->pre_loaded_attributes, $name, $default);
	...
	public function register_form() {
		 ...
		 * Add Generator Forms
		 */
		wpdev_register_form("shortcode_{$this->id}", array(
		 ...
		 * Add Customize Forms
		 */
		wpdev_register_form("customize_{$this->id}", array(
		...
		foreach ($fields as $field_slug => &$field) {
			...
			if ($field['type'] === 'group') {
				...
				foreach ($field['fields'] as $sub_field_slug => &$sub_field) {
					...
					$additional_state[$sub_field_slug] = wpdev_request($sub_field_slug, wpdev_get_isset($defaults, $sub_field_slug));
			...
			$required = wpdev_get_isset($field, 'required');

			if (wpdev_get_isset($field, 'required')) {
			...
			$state[$field_slug] = wpdev_request($field_slug, wpdev_get_isset($defaults, $field_slug));

		} // end foreach;

		$fields['shortcode_result'] = array(
			'type'            => 'note',
			'wrapper_classes' => 'sm:wpdev-block',
			'desc'            => '<div class="wpdev-w-full"><span class="wpdev-my-1 wpdev-text-2xs wpdev-uppercase wpdev-font-bold wpdev-block">' . __('Result', 'wpdev') . '</span><pre v-html="shortcode" id="wpdev-shortcode" style="overflow-x: scroll !important;" class="wpdev-text-center wpdev-p-4 wpdev-m-0 wpdev-mt-2 wpdev-rounded wpdev-content-center wpdev-bg-gray-800 wpdev-text-white wpdev-font-mono wpdev-border wpdev-border-solid wpdev-border-gray-300 wpdev-max-h-screen wpdev-overflow-x-scroll"></pre></div>',
		);

		$fields['submit_copy'] = array(
			'type'            => 'submit',
			'title'           => __('Copy Shortcode', 'wpdev'),
			'value'           => 'edit',
			'classes'         => 'button button-primary wpdev-w-full wpdev-copy',
			'wrapper_classes' => 'wpdev-items-end',
			'html_attr'       => array(
				'data-clipboard-action' => 'copy',
				'data-clipboard-target' => '#wpdev-shortcode',
			),
		);

		$form = new \WPDevFramework\UI\Form($this->id, $fields, array(
			'views'                 => 'admin-pages/fields',
			'classes'               => 'wpdev-modal-form wpdev-widget-list wpdev-striped wpdev-m-0 wpdev-mt-0 wpdev-w-full',
			...
			'html_attr'             => array(
				'data-wpdev-app' => "{$this->id}_generator",
	...
	public function render_customize_modal() {
		...
		$fields['hide'] = array(
			'type'            => 'toggle',
			'title'           => __('Hide Element', 'wpdev'),
			'desc'            => __('Be careful. Hiding an element from the account page might remove important functionality from your customers\' reach.', 'wpdev'),
		...
		foreach ($fields as $field_slug => &$field) {
			...
			$value = wpdev_get_isset($saved_settings, $field_slug, null);
		...
		$form = new \WPDevFramework\UI\Form($this->id, $fields, array(
			...
			'field_wrapper_classes' => 'wpdev-w-full wpdev-box-border wpdev-items-center wpdev-flex wpdev-justify-between wpdev-p-4 wpdev-m-0 wpdev-border-t wpdev-border-l-0 wpdev-border-r-0 wpdev-border-b-0 wpdev-border-gray-300 wpdev-border-solid',
			'html_attr'             => array(
				'data-wpdev-app' => "{$this->id}_customize",
				'data-state'  => wpdev_convert_to_state($state),
			),
		));
	...
	public function handle_customize_modal() {
		...
		if (wpdev_request('submit') !== 'restore') {
			...
			foreach ($fields as $field_slug => $field) {
				...
				$setting = wpdev_request($field_slug, false);
	...
	public function get_id() {
		...
		return sprintf('wpdev/%s', $this->id);
	...
	public function get_shortcode_id() {
		...
		return str_replace('-', '_', sprintf('wpdev_%s', $this->id));
	...
	public function display($atts) {
		 ...
		 */
		$atts['className'] = trim('wpdev-' . $this->id . ' wpdev-element ' . wpdev_get_isset($atts, 'className', ''));
	...
	public function should_display_customize_controls() {
		...
		return apply_filters('wpdev_element_display_super_admin_notice', current_user_can('manage_network'), $this);
	...
	public function as_inline_content($screen_id, $hook = 'admin_notices', $atts = array()) {
		...
		if (!function_exists('get_current_screen')) {
			...
			_doing_it_wrong(__METHOD__, __('An element can not be loaded as inline content unless the get_current_screen() function is already available.', 'wpdev'), '2.0.0');
		...
		if (wpdev_get_isset($atts, 'hide', $this->hidden_by_default)) {
			...
			$control_classes = 'wpdev-customize-mode wpdev-opacity-25';

		} // end if;

		add_action($hook, function() use ($atts, $control_classes) {

			echo '<div class="wpdev-inline-widget">';

				echo '<div class="wpdev-inline-widget-body ' . $control_classes . '">';
		...
		do_action("wpdev_{$this->id}_scripts", null, $this);
	...
	public function save_widget_settings($settings) {
		...
		$key = wpdev_replace_dashes($this->id);

		wpdev_save_setting("widget_{$key}_settings", $settings);
	...
	public function get_widget_settings() {
		...
		return wpdev_get_setting("widget_{$key}_settings", array());

	} // end get_widget_settings;

	/**
	 * Resolve a registered widget datasource (K6-005).
	 *
	 * @since 2.5.0
	 *
	 * @param string               $id      Datasource slug.
	 * @param array<string, mixed> $context Optional context passed to the callback.
	 * @return mixed
	 */
	protected function widget_datasource( $id, array $context = array() ) {

		if ( ! function_exists( 'wpdev_widget_datasource' ) ) {
			return null;
		}

		return wpdev_widget_datasource( $id, $context );

	} // end widget_datasource;
	...
	public function as_metabox($screen_id, $position = 'normal', $atts = array()) {
		...
		add_meta_box(
			"wpdev-{$this->id}-element",
			$this->get_title(),
			function() use ($atts, $control_classes) {

				echo '<div class="wpdev-metabox-widget ' . $control_classes . '">';
	...
	public function super_admin_notice() {
		...
		if ($should_display) {
			...
			$message = __('<a class="wubox wpdev-no-underline" title="Customize" href="%1$s">Customize this element</a>, or <a class="wubox wpdev-no-underline" title="Shortcode" href="%2$s">generate a shortcode</a> to use it on the front-end!', 'wpdev');

			$message .= wpdev_tooltip(__('You are seeing this because you are a super admin', 'wpdev'));

			$link_shortcode = wpdev_get_form_url("shortcode_{$this->id}");
			$link_customize = wpdev_get_form_url("customize_{$this->id}");
			...
			$html = '
				<div class="wpdev-styling">
						<div class="wpdev-widget-inset">
							<div class="wubox wpdev-no-underline wpdev-p-4 wpdev-bg-gray-200 wpdev-block wpdev-mt-4 wpdev-text-center wpdev-text-sm wpdev-text-gray-600 wpdev-m-auto wpdev-border-solid wpdev-border-0 wpdev-border-t wpdev-border-gray-400">
								' . $text . '
							</div>
					</div>
				</div>
	...
	public function is_preview() {
		...
		if (did_action('init')) {
			...
			$is_preview = wpdev_request('preview') && current_user_can('edit_posts');

		} // end if;

		return apply_filters('wpdev_element_is_preview', false, $this);
