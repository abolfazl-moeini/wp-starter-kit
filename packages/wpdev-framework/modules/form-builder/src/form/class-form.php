...
class Form implements \JsonSerializable {
	...
	public function __construct($id, $fields, $atts = array()) {
		...
		$this->atts = apply_filters("wpdev_{$id}_form_atts", wp_parse_args($atts, array(
			'id'                    => $id,
			'method'                => 'post',
			'before'                => '',
			'after'                 => '',
	...
	/**
	 * Casts fields to \WPDevFramework\UI\Fields and stores them on private list.
	 *
	 * @since 2.0.0
	 *
	...
	public function set_fields($fields) {
		 ...
		 * @param array $fields List of fields of the form.
		 */
		$fields = apply_filters("wpdev_{$id}_form_fields", $fields);
	...
	public function render() {
		...
		foreach ($this->get_fields() as $field_slug => $field) {
			...
			if (wpdev_get_isset($field->wrapper_html_attr, 'id') === false) {
			...
			/*
			 * K2-02: resolve field templates through the unified field resolver.
			 * wpdev_field_view() treats $this->views as a raw view root and
			 * applies the same underscore->hyphen normalization as before.
			 */
			if (function_exists('wpdev_field_view')) {
				$field_view     = wpdev_field_view($this->views, $field->type);
				$field_fallback = wpdev_field_view($this->views, 'text');
			} else {
				$field_view     = "{$this->views}/field-" . $field->get_template_name();
				$field_fallback = "{$this->views}/field-text";
			}

			wpdev_get_template($field_view, array(
		...
		wpdev_get_template("{$this->views}/form", $variables);
