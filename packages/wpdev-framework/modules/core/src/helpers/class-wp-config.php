...
class WP_Config {
  ...
  * @since 2.0.0
  *
  * @param string     $constant The name of the constant. e.g. WPDev_CONSTANT.
 ...
 public function inject_wp_config_constant($constant, $value) {
		...
		if (!is_writeable($config_path)) {
			...
			return new \WP_Error('not-writeable', sprintf(__('The file %s is not writable', 'wpdev'), $config_path));
		...
		$content = str_pad(sprintf("define( '%s', '%s' );", $constant, $value), 50) . '// Automatically injected by WPDev;';
		...
		if ($line === false) {
			...
			if ($hook_line === false) {
				...
				return new \WP_Error('unknown-wpconfig', __("WPDev can't recognize your wp-config.php, please revert it to original state for further process.", 'wpdev'));
	...
	public function find_reference_hook_line($config) {
		 ...
		 */
		$patterns = apply_filters('wpdev_wp_config_reference_hook_line_patterns', array(
