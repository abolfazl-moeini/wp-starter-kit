...
add_action(
	...
	static function () {
		...
		$duplicate_file = wpdev_examples_file( 'sites/src/duplication/duplicate.php' );

		if ( '' !== $duplicate_file && is_readable( $duplicate_file ) ) {
...
class Site_Duplicator {
	...
	public static function duplicate_site($from_site_id, $title, $args = array()) {
		...
		if (is_wp_error($duplicate_site)) {
			...
			$message = sprintf(__('Attempt to duplicate site %1$d failed: %2$s', 'wpdev'), $from_site_id, $duplicate_site->get_error_message());

			wpdev_log_add('site-duplication', $message, LogLevel::ERROR);
		...
		$message = sprintf(__('Attempt to duplicate site %1$d successful - New site id: %2$d', 'wpdev'), $from_site_id, $duplicate_site);

		wpdev_log_add('site-duplication', $message);
	...
	public static function override_site($from_site_id, $to_site_id, $args = array()) {
		...
		$to_site = wpdev_get_site($to_site_id);
		...
		if (is_wp_error($duplicate_site_id)) {
			...
			// translators: %s id the template site id and %s is the error message returned.
			$message = sprintf(__('Attempt to override site %1$d with data from site %2$d failed: %3$s', 'wpdev'), $from_site_id, $to_site_id, $duplicate_site_id->get_error_message());
		...
		$new_to_site = wpdev_get_site($duplicate_site_id);
		...
		if ($saved) {
			...
			$message = sprintf(__('Attempt to override site %1$d with data from site %2$d successful.', 'wpdev'), $from_site_id, $duplicate_site_id);
	...
	protected static function process_duplication($args) {
		...
		$args = wp_parse_args($args, array(
			...
			'copy_files'   => wpdev_get_setting('copy_media', true),
		...
		if (!$args->from_site_id) {
			...
			return new \WP_Error('from_site_id_required', __('You need to provide a valid site to duplicate.', 'wpdev'));
		...
		if (!is_numeric($args->to_site_id)) {
			...
			return new \WP_Error('site_creation_failed', __('An attempt to create a new site failed.', 'wpdev'));
		 ...
		 * @return void
		 */
		do_action('wpdev_duplicate_site', array(
	...
	public static function create_admin($email, $domain) {
		...
		if (!$user_id) { // Create a new user with a random password
			...
			if (false === $user_id) {
				...
				return new \WP_Error('user_creation_error', __('We were not able to create a new admin user for the site being duplicated.', 'wpdev'));
