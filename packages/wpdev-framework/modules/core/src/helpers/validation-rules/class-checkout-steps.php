...
use WPDevFramework\Managers\Signup_Fields_Manager;
  ...
  public function check($value) : bool {
    ...
    foreach ($required_fields_list as $field_slug) {
      ...
      if (!in_array($field_slug, $all_fields_list, true)) {
        ...
        $this->message = sprintf(__('The %s field must be present in at least one of the checkout form steps.', 'wpdev'), wpdev_slug_to_name($field_slug));
    ...
    $submittable_field_types = apply_filters(
      'wpdev_checkout_step_validation_submittable_field_types',
      array(
        'submit_button',
        'pricing_table',
        'template_selection',
    ...
    foreach ($value as $step) {
      ...
      $found_submittable_field_types = \WPDevFramework\Dependencies\Arrch\Arrch::find($step['fields'], array(
        'where'    => array(
      ...
      if (empty($found_submittable_field_types)) {
        ...
        $this->message = sprintf(__('The %s step is missing a submit field', 'wpdev'), $step['name']);
