...
class Screenshot {
	...
	public static function api_url($domain): string {
		...
		$url = wpdev_with_license_key(sprintf('https://api.wpdev.ir/screenshot?url=%s', $domain));
 ...
 public static function save_image_from_url($url) {
		...
		if (wp_remote_retrieve_response_code($response) !== 200) {
			...
			wpdev_log_add('screenshot-generator', $log_prefix . wp_remote_retrieve_response_message($response), LogLevel::ERROR);
		...
		if (is_wp_error($response)) {
			...
			wpdev_log_add('screenshot-generator', $log_prefix . $response->get_error_message(), LogLevel::ERROR);
		...
		if (strncmp($response['body'], "\x89\x50\x4e\x47\x0d\x0a\x1a\x0a", strlen("\x89\x50\x4e\x47\x0d\x0a\x1a\x0a")) !== 0) {
			...
			wpdev_log_add('screenshot-generator', $log_prefix . __('Result is not a PNG file.', 'wpdev'), LogLevel::ERROR);
		...
		if (!empty($upload['error'])) {
			...
			wpdev_log_add('screenshot-generator', $log_prefix . json_encode($upload['error']), LogLevel::ERROR);
		...
		wpdev_log_add('screenshot-generator', $log_prefix . __('Success!', 'wpdev'));
