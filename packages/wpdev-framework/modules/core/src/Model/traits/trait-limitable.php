...
trait Limitable {
	...
	public function get_limitations($waterfall = true, $skip_self = false) {
		...
		$cached_version = wpdev_get_isset($this->_limitations, $cache_key);
		...
		if (did_action('muplugins_loaded') === false) {
			...
			$modules_data = $this->get_meta('wpdev_limitations', array());
	...
	public function sync_plugins() {
		...
		foreach ($sites as $site_object) {
			...
			if ($plugins_to_deactivate) {
				...
				wpdev_async_deactivate_plugins($site_id, array_keys($plugins_to_deactivate));

			} // end if;

			if ($plugins_to_activate) {

				wpdev_async_activate_plugins($site_id, array_keys($plugins_to_activate));
	 ...
	 *
	 * This is called on the handle_save method of the modules/admin-page-builder/src/admin/class-edit-admin-page.php
	 * for all models that have the trait Limitable.
	 *
	 * @see modules/admin-page-builder/src/admin/class-edit-admin-page.php
	 *
	 * @since 2.0.0
	 * @return void
	 */
	public function handle_limitations() {
		...
		foreach ($limitations as $limitation_id => $class_name) {
			...
			$module = wpdev_get_isset($saved_limitations, $limitation_id, array());
		...
		if ($this->model !== 'product') {
			 ...
			 */
			$limitations = wpdev_array_recursive_diff($modules_to_save, $current_limitations->to_array());

		} elseif ($this->model === 'product' && $this->get_type() !== 'plan') {

			$limitations = wpdev_array_recursive_diff($modules_to_save, Limitations::get_empty()->to_array());
		...
		$this->meta['wpdev_limitations'] = $limitations;
	...
	public function get_applicable_product_slugs() {
		...
		if (!empty($membership)) {
			...
			$slugs = array_column(array_map('wpdev_cast_model_to_array', array_column($membership->get_all_products(), 'product')), 'slug'); // WOW
