...
abstract class Base_Model implements \JsonSerializable {
	...
	public function get_hash($field = 'id') {
		...
		if (!is_numeric($value)) {
			...
			_doing_it_wrong(__METHOD__, __('You can only use numeric fields to generate hashes.', 'wpdev'), '2.0.0');
	...
	public function attributes($atts) {
		...
		foreach ($atts as $key => $value) {
			...
			$mapping = wpdev_get_isset($this->_mappings, $key);
	...
	public function has_running_jobs() {
		...
		$jobs = wpdev_get_scheduled_actions(array(
	...
	public function save() {
		...
		$meta = wpdev_get_isset($data, 'meta', array());
		 ...
		 */
		$meta = apply_filters("wpdev_{$this->model}_meta_pre_save", $meta, $data_unserialized, $this);
		 ...
		 * @param Base_Model $this The object instance.
		 */
		$data = apply_filters("wpdev_{$this->model}_pre_save", $data, $data_unserialized, $this);
		 ...
		 */
		wp_cache_delete($this->get_id(), "wpdev-{$this->model}s");
		 ...
		 * @param Base_Model $this The object instance.
		 */
		do_action('wpdev_model_post_save', $this->model, $data, $data_unserialized, $this);
		 ...
		 */
		do_action("wpdev_{$this->model}_post_save", $data, $this, $new);
	...
	public function delete() {
		...
		if (!$this->get_id()) {
			...
			return new \WP_Error("wpdev_{$this->model}_delete_unsaved_item", __('Item not found.', 'wpdev'));
		 ...
		 * @param Base_Model $this The object instance.
		 */
		do_action("wpdev_{$this->model}_pre_delete", $this);
		 ...
		 * @param Base_Model $this   The object instance.
		 */
		do_action("wpdev_{$this->model}_post_delete", $result, $this);
	...
	private function get_meta_type_name() {
		...
		$prefix = ! empty( $query_class->prefix ) ? $query_class->prefix : '';

		if ( class_exists( 'WPDev\\Database\\Engine\\Meta_Type_Compat' ) ) {
			return \WPDevFramework\Database\Engine\Meta_Type_Compat::meta_type_for_item(
		...
		$table = ! empty( $prefix )
			? "{$prefix}_{$query_class->item_name}"
			: $query_class->item_name;
	...
	protected function is_meta_available() {
		...
		if (!$this->get_meta_table_name()) {
			...
			// _doing_it_wrong(__METHOD__, __('This model does not support metadata.', 'wpdev'), '2.0.0');
		...
		if (!$this->get_id() && !$this->_mocked) {
			...
			// _doing_it_wrong(__METHOD__, __('Model metadata only works for already saved models.', 'wpdev'), '2.0.0');
	...
	public function update_meta_batch($meta) {
		...
		if (!is_array($meta)) {
			...
			_doing_it_wrong(__METHOD__, __('This method expects an array as argument.', 'wpdev'), '2.0.0');
	...
	public function jsonSerialize() {
		...
		if (wp_doing_ajax() && wpdev_request('action') === 'wpdev_search') {
	...
	public function get_date_created() {
		...
		if (!wpdev_validate_date($this->date_created)) {

			return wpdev_get_current_time('mysql');
	...
	public function get_date_modified() {
		...
		if (!wpdev_validate_date($this->date_modified)) {
	...
	public function get_formatted_amount($key = 'amount') {
		...
		if (is_numeric($value)) {
			...
			return wpdev_format_currency($value);
	...
	public function get_formatted_date($key = 'date_created') {
		...
		if (wpdev_validate_date($value)) {

			return date_i18n(get_option('date_format'), wpdev_date($value)->format('U'));
	 ...
	 *
	 * @since 2.0.0
	 * @return \WPDevFramework\Model\Base_Model
	 */
	public function duplicate() {
		...
		if (method_exists($clone, 'set_date_created')) {
			...
			$clone->set_date_created(wpdev_get_current_time('mysql'));
	...
	public function lock() {
		...
		return $this->update_meta('wpdev_lock', true);
	...
	public function is_locked() {
		...
		return $this->get_meta('wpdev_lock', false);
	...
	public function unlock() {
		...
		return $this->delete_meta('wpdev_lock');
