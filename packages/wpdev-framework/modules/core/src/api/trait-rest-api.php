...
trait Rest_Api {
	...
	public function enable_rest_api() {
		...
		$is_enabled = \WPDevFramework\API::get_instance()->is_api_enabled();
	...
	public function register_routes_general() {
		...
		if (!empty($routes)) {
			...
			register_rest_route(
				\WPDevFramework\API::get_instance()->get_namespace(),
				'/' . $this->get_rest_base(),
				$routes,
				true
			);
		...
		do_action('wpdev_rest_register_routes_general', $routes, $this->get_rest_base(), 'create', $this);
	...
	public function register_routes_with_id() {
		...
		do_action('wpdev_rest_register_routes_with_id', $routes, $this->get_rest_base(), 'update', $this);
 ...
 public function get_item_rest($request) {
		...
		if (empty($item)) {
			...
			return new \WP_Error("wpdev_rest_{$this->slug}_invalid_id", __('Item not found.', 'wpdev'), array('status' => 404));
 ...
 public function create_item_rest($request) {
		...
		$saver_function = "wpdev_create_{$model_name}";
		...
		if (!$saved) {
			...
			return new \WP_Error("wpdev_rest_{$this->slug}", __('Something went wrong (Code 1).', 'wpdev'), array('status' => 400));
 ...
 public function update_item_rest($request) {
		...
		$id = wpdev_get_isset($request->get_url_params(), 'id');
		...
		foreach ($params as $param => $value) {
			...
			if ($param === 'meta') {
				...
				$error_message = sprintf(
					...
					__('The %1$s object does not have a %2$s method', 'wpdev'),
					get_class($item),
					$set_method
				);

				return new \WP_Error(
					"wpdev_rest_{$this->slug}_invalid_set_method",
					$error_message,
					array('status' => 400)
				);
		...
		if (!$saved) {
			...
			return new \WP_Error("wpdev_rest_{$this->slug}", __('Something went wrong (Code 2).', 'wpdev'));
 ...
 public function delete_item_rest($request) {
	...
	public function get_items_permissions_check($request) {
		...
		if (!\WPDevFramework\API::get_instance()->check_authorization($request)) {
		 ...
		 */
		return apply_filters('wpdev_rest_get_items', true, $this->get_rest_base(), $this);
	...
	public function create_item_permissions_check($request) {
		 ...
		 * @param Base_Manager $this      The object instance.
		 */
		return apply_filters('wpdev_rest_create_item', true, $this->get_rest_base(), $this);
	...
	public function get_item_permissions_check($request) {
		 ...
		 * @param Base_Manager $this      The object instance.
		 */
		return apply_filters('wpdev_rest_get_item', true, $this->get_rest_base(), $this);
	...
	public function update_item_permissions_check($request) {
		 ...
		 * @param Base_Manager $this      The object instance.
		 */
		return apply_filters('wpdev_rest_update_item', true, $this->get_rest_base(), $this);
	...
	public function delete_item_permissions_check($request) {
		 ...
		 * @param Base_Manager $this      The object instance.
		 */
		return apply_filters('wpdev_rest_delete_item', true, $this->get_rest_base(), $this);
	...
	public function get_arguments_schema($edit = false) {
		...
		$schema = wpdev_rest_get_endpoint_schema($this->model_class, $edit ? 'update' : 'create', true);
	...
	public function filter_schema_arguments($args) {
		 ...
		 */
		apply_filters('wpdev_before_' . $this->slug . '_api_arguments', $args, $this);
		...
		$remove_status = apply_filters("wpdev_api_{$this->slug}_remove_status", array(
			'broadcast',
			'membership',
			'product',
			'payment',
		...
		$remove_slug = apply_filters("wpdev_api_{$this->slug}_remove_slug", array(
			'broadcast',
			'product',
			'WPDevFramework\WCExport\FeatureName\Database\Checkout_Form',
			'event',
		));
		 ...
		 * @param object $this This manager.
		 */
		apply_filters('wpdev_after_' . $this->slug . '_api_arguments', $args, $this);
