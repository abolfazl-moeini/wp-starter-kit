...
trait WP_CLI {
	...
	public function set_wp_cli_enabled_sub_commands() {
		 ...
		 */
		$params_to_remove = apply_filters('wpdev_cli_params_to_remove', array(
			'id',
			'model',
		));
		...
		foreach ($sub_commands as $sub_command => &$sub_command_data) {
			...
			if (in_array($sub_command, array('get', 'update', 'delete'), true)) {
				...
				$sub_command_data['synopsis'][] = array(
					'name'        => 'id',
					'type'        => 'positional',
					'description' => __('The id for the resource.', 'wpdev'),
					'optional'    => false,
				);
			...
			if (in_array($sub_command, array('list', 'update', 'create'), true)) {
				...
				$explanation_list = wpdev_rest_get_endpoint_schema($this->model_class, 'update');

				foreach ($params as $name) {

					$explanation = wpdev_get_isset($explanation_list, $name, array());

					$type = wpdev_get_isset($explanation, 'type', 'assoc');

					$field = array(
						'name'        => $name,
						'description' => wpdev_get_isset($explanation, 'description', __('No description found.', 'wpdev')),
						'optional'    => !wpdev_get_isset($explanation, 'required'),
						'type'        => 'assoc',
					);

					$options = wpdev_get_isset($explanation, 'options', array());
			...
			if (in_array($sub_command, array('create', 'update'), true)) {
				...
				$sub_command_data['synopsis'][] = array(
					'name'        => 'porcelain',
					'type'        => 'flag',
					'description' => __('Output just the id when the operation is successful.', 'wpdev'),
					'optional'    => true,
				);
			...
			if (in_array($sub_command, array('list', 'get'), true)) {
				...
				$sub_command_data['synopsis'][] = array(
					'name'        => 'format',
					'type'        => 'assoc',
					'description' => __('Render response in a particular format.', 'wpdev'),
					'optional'    => true,
					'default'     => 'table',
					'options'     => array(
						'table',
				...
				$sub_command_data['synopsis'][] = array(
					'name'        => 'fields',
					'type'        => 'assoc',
					'description' => __('Limit response to specific fields. Defaults to id, name', 'wpdev'),
					'optional'    => true,
		...
		$this->wp_cli_enabled_sub_commands = apply_filters(
			'wpdev_wp_cli_enabled_sub_commands',
