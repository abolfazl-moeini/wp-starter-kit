...
use \WPDevFramework\Dependencies\Ifsnop\Mysqldump\Mysqldump as MySQLDump;
use \WPDevFramework\UI\Template_Previewer;
use \WPDevFramework\Models\Checkout_Form;
use \WPDevFramework\Checkout\Legacy_Checkout;
...
use \WPDevFramework\Managers\Domain_Manager;
use \WPDevFramework\Managers\Limitation_Manager;
...
class Migrator extends Base_Installer {
	...
	public function init() {
		...
		if ($this->is_migration_screen()) {
			...
			$this->session = wpdev_get_session('migrator');
		 ...
		 */
		\WPDevFramework\Async_Calls::register_listener('parallel_installers', array($this, 'handle_parallel_installers'));

		\WPDevFramework\Async_Calls::install_listeners();
	...
	public function handle_parallel_installers() {
		...
		do_action('wp_ajax_wpdev_setup_install'); // phpcs:ignore
	 ...
	 * @since 2.0.7
	 *
	 * @param \WPDevFramework\Session $session The session handler object.
	...
	public function on_shutdown($session, $dry_run, $installer) {
		...
		wpdev_log_add(Migrator::LOG_FILE_NAME, sprintf($message, $installer), LogLevel::ERROR);
	...
	protected function is_migration_screen() {
		...
		return wpdev_request('page') === 'wpdev-setup' && wpdev_request('step') === 'migration';
	...
	public static function is_migration_done() {
		...
		return get_network_option(null, 'wpdev_is_migration_done', false);

	} // end is_migration_done;

	/**
	 * Check if we are running on a network that runs wpdev 1.X
	 *
	 * @since 2.0.0
	 * @return boolean
	 */
	...
	public function get_steps($force_all = false) {
		...
		if ($dry_run && !$force_all) {
			...
			$steps['dry_run_check'] = array(
				'title'       => __('Pre-Migration Check', 'wpdev'),
				'description' => __('Runs all migrations in a sand-boxed environment to see if it hits an error.', 'wpdev'),
				'help'        => wpdev_get_documentation_url('migration-errors'),
				'pending'     => __('Pending', 'wpdev'),
				'installing'  => __('Checking...', 'wpdev'),
		...
		if (!$dry_run) {
			...
			$steps['backup'] = array(
				'title'       => __('Prepare for Migration', 'wpdev'),
				'description' => __('Verifies the data before going forward with the migration.', 'wpdev'),
				'pending'     => __('Pending', 'wpdev'),
				'installing'  => __('Preparing...', 'wpdev'),
				'success'     => __('Success!', 'wpdev'),
				'help'        => wpdev_get_documentation_url('migration-errors'),
				'done'        => false,
			);
		...
		$steps['settings'] = array(
			...
			'description' => __('Migrates the settings from the older version.', 'wpdev'),
			'help'        => wpdev_get_documentation_url('migration-errors'),
			'done'        => false,
		);

		$steps['products'] = array(
			'title'       => __('Plans to Products', 'wpdev'),
			'description' => __('Converts the old plans into products.', 'wpdev'),
			'help'        => wpdev_get_documentation_url('migration-errors'),
			'done'        => false,
		);

		$steps['customers'] = array(
			'title'       => __('Users to Customers', 'wpdev'),
			'description' => __('Creates customers based on the existing users.', 'wpdev'),
			'help'        => wpdev_get_documentation_url('migration-errors'),
			'done'        => false,
		);

		$steps['memberships'] = array(
			'title'       => __('Subscriptions to Memberships', 'wpdev'),
			'description' => __('Converts subscriptions into Memberships.', 'wpdev'),
			'help'        => wpdev_get_documentation_url('migration-errors'),
			'done'        => false,
		);

		$steps['transactions'] = array(
			'title'       => __('Transactions to Payments & Events', 'wpdev'),
			'description' => __('Converts transactions into payments and events.', 'wpdev'),
			'help'        => wpdev_get_documentation_url('migration-errors'),
			'done'        => false,
		);

		$steps['discount_codes'] = array(
			'title'       => __('Coupons to Discount Codes', 'wpdev'),
			'description' => __('Converts coupons into discount codes.', 'wpdev'),
			'help'        => wpdev_get_documentation_url('migration-errors'),
			'done'        => false,
		);

		$steps['sites'] = array(
			'title'       => __('Customer Sites', 'wpdev'),
			'description' => __('Adjusts existing customer sites.', 'wpdev'),
			'installing'  => __('Making Adjustments...', 'wpdev'),
			'help'        => wpdev_get_documentation_url('migration-errors'),
			'done'        => false,
		);

		$steps['site_templates'] = array(
			'title'       => __('Sites Templates', 'wpdev'),
			'description' => __('Adjusts existing site templates.', 'wpdev'),
			'installing'  => __('Making Adjustments...', 'wpdev'),
			'help'        => wpdev_get_documentation_url('migration-errors'),
			'done'        => false,
		);
		...
		$steps['domains'] = array(
			'title'       => __('Mapped Domains', 'wpdev'),
			'description' => __('Converts mapped domains.', 'wpdev'),
			'help'        => wpdev_get_documentation_url('migration-errors'),
			'done'        => false,
		);
		...
		$steps['forms'] = array(
			...
			'description' => __('Creates a checkout form based on the existing signup flow.', 'wpdev'),
			'help'        => wpdev_get_documentation_url('migration-errors'),
			'done'        => false,
		);

		$steps['emails'] = array(
			'title'       => __('Emails & Broadcasts', 'wpdev'),
			'description' => __('Converts the emails and broadcasts.', 'wpdev'),
			'help'        => wpdev_get_documentation_url('migration-errors'),
			'done'        => false,
		);

		$steps['webhooks'] = array(
			'title'       => __('Webhooks', 'wpdev'),
			'description' => __('Migrates existing webhooks.', 'wpdev'),
			'help'        => wpdev_get_documentation_url('migration-errors'),
			'done'        => false,
		);

		$steps['other'] = array(
			'title'       => __('Other Migrations', 'wpdev'),
			'description' => __('Other migrations that don\'t really fit anywhere else.', 'wpdev'),
			'help'        => wpdev_get_documentation_url('migration-errors'),
			'done'        => false,
		);
		 ...
		 * @param \WPDevFramework\Migrator $this This class.
		 */
		$steps = apply_filters('wpdev_get_migration_steps', $steps, $this);
	...
	public function handle($status, $installer, $wizard) {
		...
		global $wpdb, $wpdev_migrator_current_installer;

		/*
		 * Set the dry run status, which is true by default.
		 */
		$this->dry_run = wpdev_request('dry-run', true);

		$callable = array($this, "_install_{$installer}");

		$callable = apply_filters("wpdev_installer_{$installer}_callback", $callable, $installer);
		...
		*/
		...
		if ($this->is_parallel()) {
			...
			$opening_log_message .= sprintf(' This is a parallel request with a page attribute of %d and a per_page attribute of %d', wpdev_request('page', 0), wpdev_request('per_page', 0));

		} // end if;

		wpdev_log_add(Migrator::LOG_FILE_NAME, $opening_log_message);
		...
		if (is_wp_error($this->server_bypass_status)) {
			...
			wpdev_log_add(Migrator::LOG_FILE_NAME, $log_message, LogLevel::ERROR);

		} // end if;

		$session = wpdev_get_session('migrator');
		...
		try {
			...
			call_user_func($callable, wpdev_request('per_page'), wpdev_request('page'));
			 ...
			 */
			wpdev_log_add(Migrator::LOG_FILE_NAME, $e->__toString(), LogLevel::ERROR);
	 ...
	 * @param \WPDevFramework\Session $session THe WPDev session object.
	...
	public function handle_error_messages($e, $session, $dry_run = true, $installer = 'none') {
		...
		global $wpdev_migrator_current_installer;

		$caller = $dry_run ? $wpdev_migrator_current_installer : $installer;

		// Translators: %s is the name of the installer.
		$error_nice_message = sprintf(__('Critical error found when migrating "%s".', 'wpdev'), $caller);
	...
	public function log_ids_of_interest() {
		...
		if (!is_array($this->ids_of_interest)) {
			...
			wpdev_log_add(Migrator::LOG_FILE_NAME, 'The list of IDs of interested got corrupted, this might indicate problems with one or more migrations', LogLevel::ERROR);
		...
		foreach ($this->ids_of_interest as $key => $ids) {
			...
			wpdev_log_add(Migrator::LOG_FILE_NAME, $message, LogLevel::WARNING);
	...
	public function add_id_of_interest($id_or_ids, $reason, $installer) {
		...
		$id_list = wpdev_get_isset($this->ids_of_interest, $id_list_key, array());
	...
	public function _install_dry_run_check() {
		...
		foreach ($all_steps as $installer => $step) {
			...
			$wpdev_migrator_current_installer = $installer;
	...
	public function _install_backup() {
		...
		$folder = wpdev_maybe_create_folder('wpdev-backup');

		$file_name = $folder . date_i18n('Y-m-d-his') . '-wpdev-dump.sql';
	...
	public function get_old_setting($setting, $default = false) {
		...
		$value = wpdev_get_isset($settings, $setting, $default);
	...
	protected function _install_settings() {
		 ...
		 */
		wpdev_save_option("v1_settings_{$random_key}", $settings);
		 ...
		 * @since 2.0.7
		 */
		$to_migrate['enable_multiple_accounts'] = class_exists('\wpdev_Multiple_Logins');
		...
		*/
		...
		$top_bar_settings = array(
			...
			'button_text'                 => $this->get_old_setting('top-bar-button-text', __('Use this Template', 'wpdev')),
		 ...
		 */
		$status = \WPDevFramework\Settings::get_instance()->save_settings($to_migrate);
	...
	public function fake_register_settings($to_migrate = array()) {
		...
		add_filter('wpdev_settings_section_core_fields', function($fields) use ($to_migrate) {

			$all_keys = \WPDevFramework\Settings::get_instance()->get_all();
		...
		\WPDevFramework\Settings::get_instance()->default_sections();
	...
	protected function _install_products() {
		...
		if ( ! function_exists( 'wpdev_create_product' ) ) {
			return;
		}

		$settings = wpdev_get_option('settings');
		...
		foreach ($plans as $plan) {
			 ...
			 */
			if ( function_exists( 'wpdev_get_product_by_slug' ) && wpdev_get_product_by_slug( $plan->slug ) ) {
			...
			$product_data['description']   = get_post_meta($plan->ID, 'wpdev_description', true);
			$product_data['setup_fee']     = get_post_meta($plan->ID, 'wpdev_setup_fee', true);
			$product_data['featured_plan'] = (bool) get_post_meta($plan->ID, 'wpdev_top_deal', true);
			$product_data['feature_list']  = get_post_meta($plan->ID, 'wpdev_feature_list', true);
			$product_data['customer_role'] = get_post_meta($plan->ID, 'wpdev_role', true);
			$product_data['list_order']    = get_post_meta($plan->ID, 'wpdev_order', true);

			/*
			 * Trial
			 */
			$default_trial_value                 = $this->get_old_setting('trial', 0);
			$plan_trial_value                    = get_post_meta($plan->ID, 'wpdev_trial', true);
			...
			$active                 = !(bool) get_post_meta($plan->ID, 'wpdev_hidden', true);
			$product_data['active'] = $active;

			$is_free = get_post_meta($plan->ID, 'wpdev_free', true);
			...
			if ($is_free) {
				...
				$price_month = get_post_meta($plan->ID, 'wpdev_price_1', true);
				...
				$price_3_month = get_post_meta($plan->ID, 'wpdev_price_3', true);
				...
				$price_12_month = get_post_meta($plan->ID, 'wpdev_price_12', true);
				...
				$is_contact_us                = (bool) get_post_meta($plan->ID, 'wpdev_is_contact_us', true);
			 ...
			 * Creates product
			 */
			$product = wpdev_create_product($product_data);
			 ...
			 * Migrate Quota
			 */
			$quotas              = (array) get_post_meta($plan->ID, 'wpdev_quotas', true);
			$disabled_post_types = array_keys((array) get_post_meta($plan->ID, 'wpdev_disabled_post_types', true));
			$allowed_plugins     = (array) get_post_meta($plan->ID, 'wpdev_allowed_plugins', true);
			$allowed_themes      = (array) get_post_meta($plan->ID, 'wpdev_allowed_themes', true);
			$has_custom_domain   = (bool) get_post_meta($plan->ID, 'wpdev_custom_domain', true);

			$site_template_mode             = 'default';
			$has_template_selection_options = (bool) $this->get_old_setting('allow_template', true);
			$templates                      = array_filter(array_keys((array) get_post_meta($plan->ID, 'wpdev_templates', true)));
			$has_custom_template_list       = (bool) get_post_meta($plan->ID, 'wpdev_override_templates', true);
			$site_template_enabled          = true;
			$force_template                 = false;

			if ($has_template_selection_options === false) {

				$force_template = get_post_meta($plan->ID, 'wpdev_site_template', true);

				if ($force_template && wpdev_get_site($force_template)) {
			...
			$available_site_templates = wpdev_get_site_templates();
			...
			$unlimited_users = (bool) get_post_meta($plan->ID, 'wpdev_unlimited_extra_users', true);
			...
			$limitations = new \WPDevFramework\Objects\Limitations($limitations_modules);

			$product->update_meta('wpdev_limitations', $limitations);
	...
	protected function is_parallel() {
		...
		return wpdev_request('parallel') && wpdev_request('page') && wpdev_request('per_page');
	...
	protected function get_installer() {
		...
		return $this->dry_run ? $wpdev_migrator_current_installer : wpdev_request('installer', '');
	...
	protected function maybe_run_in_parallel($total_records, $threshold = 100) {
		...
		if ($this->is_parallel() === false) {
			...
			if ($total_records > $threshold) {
				...
				$result = \WPDevFramework\Async_Calls::run('parallel_installers', $args, $total_records, $threshold, 10);
	...
	protected function build_limit_clause() {
		...
		if ($this->dry_run) {
			...
			$page     = absint(wpdev_request('page'));
			$per_page = absint(wpdev_request('per_page'));
	...
	protected function _install_customers() {
		 ...
		 * background or not.
		 */
		$total_records = (int) $wpdb->get_var("SELECT count(ID) FROM {$wpdb->base_prefix}wpdev_subscriptions");
		...
		if ( ! function_exists( 'wpdev_create_customer' ) ) {
			return;
		}
		...
		$users = $wpdb->get_results(
			"
				SELECT
					...
					created_at
				FROM
					{$wpdb->base_prefix}wpdev_subscriptions
				{$limit_clause}
			"
		);
		// phpcs:enable
		...
		foreach ($users as $user) {
			...
			if (wpdev_get_customer_by_user_id($user->user_id)) {

				continue;

			} // end if;

			$customer = wpdev_create_customer(array(
	...
	protected function _install_memberships() {
		 ...
		 * Load dependencies.
		 */
		wpdev_require_public_function( 'date' );
		wpdev_require_public_function( 'customer' );
		wpdev_require_public_function( 'membership' );

		if ( ! function_exists( 'wpdev_create_membership' ) ) {
			return;
		}
		...
		foreach ($subscriptions as $subscription) {
			 ...
			 */
			if (wpdev_get_membership_by('migrated_from_id', $subscription->ID)) {

				continue;

			} // end if;

			$customer = wpdev_get_customer_by_user_id($subscription->user_id);

			$product = wpdev_call_if_function_exists( 'wpdev_get_product_by', null, 'migrated_from_id', absint( $subscription->plan_id ) );
			...
			$last_renewal_date_query = $wpdb->prepare("
				SELECT
					time
				FROM
					{$wpdb->base_prefix}wpdev_transactions
				WHERE
			...
			if (wpdev_validate_date($last_renewal_date)) {
			...
			if ($subscription->trial > 0) {
				 ...
				 * @since 2.0.7
				 */
				$subscription_creation_date = wpdev_date($subscription->created_at);
			...
			if (empty(wpdev_to_float($subscription->price)) || !$subscription->active_until) {
			...
			$membership = wpdev_create_membership($membership_data);
	...
	protected function _install_transactions() {
		...
		if ( ! function_exists( 'wpdev_create_payment' ) ) {
			return;
		}
		 ...
		 * background or not.
		 */
		$total_records = (int) $wpdb->get_var("SELECT count(ID) FROM {$wpdb->base_prefix}wpdev_transactions");
		...
		wpdev_require_public_function( 'payment' );
		...
		foreach ($transactions as $transaction) {
			 ...
			 * If we have already migrated, no need to do it again.
			 */
			if (wpdev_get_payment_by('migrated_from_id', $transaction->id)) {
			...
			$membership = wpdev_get_membership_by('user_id', $transaction->user_id);

			$customer = wpdev_get_customer_by_user_id($transaction->user_id);

			$product = '';

			if (isset($transaction->plan_id)) {

				$product = wpdev_call_if_function_exists( 'wpdev_get_product_by', null, 'migrated_from_id', $transaction->plan_id );

			} // end if;

			$line_item = new \WPDevFramework\Checkout\Line_Item(array(
				'product'  => $product,
				'quantity' => 1,
			));
			...
			$line_item->set_unit_price(wpdev_to_float($transaction->amount));
			$line_item->set_subtotal(wpdev_to_float($transaction->amount));
			$line_item->set_total(wpdev_to_float($transaction->amount));
			...
			$payment_data = array(
				...
				'status'             => wpdev_get_isset($map_status, $transaction->type, Payment_Status::COMPLETED),
				...
				'discount_code'      => '',
				'subtotal'           => wpdev_to_float($transaction->amount),
				'discount_total'     => 0,
				'tax_total'          => 0,
				'total'              => wpdev_to_float($transaction->amount),
			...
			$payment = wpdev_create_payment($payment_data);
	...
	protected function _install_discount_codes() {
		 ...
		 * Load dependencies.
		 */
		wpdev_require_public_function( 'discount-code' );

		if ( ! function_exists( 'wpdev_get_discount_code_by_code' ) || ! function_exists( 'wpdev_create_discount_code' ) ) {
			return;
		}
		...
		foreach ($coupons as $coupon) {
			 ...
			 * If we have already migrated, no need to do it again.
			 */
			if (wpdev_get_discount_code_by_code($coupon->slug)) {

				continue;

			} // end if;

			$wpdev_expiring_date = get_post_meta($coupon->ID, 'wpdev_expiring_date', true);

			if (!wpdev_validate_date($wpdev_expiring_date)) {

				$wpdev_expiring_date = false;

			} // end if;

			$allowed_plans = get_post_meta($coupon->ID, 'wpdev_allowed_plans', true);
			...
			if ($allowed_plans) {
				...
				foreach ($allowed_plans as $key => $plan_id) {
					...
					$product = wpdev_call_if_function_exists( 'wpdev_get_product_by', null, 'migrated_from_id', $plan_id );
			...
			$discount_code_data = array(
				'uses'             => get_post_meta($coupon->ID, 'wpdev_uses', true),
				'max_uses'         => get_post_meta($coupon->ID, 'wpdev_allowed_uses', true),
				'name'             => $coupon->name,
				'description'      => get_post_meta($coupon->ID, 'wpdev_description', true),
				'code'             => $coupon->slug,
				'type'             => get_post_meta($coupon->ID, 'wpdev_type', true),
				'value'            => get_post_meta($coupon->ID, 'wpdev_value', true),
				'setup_fee_type'   => get_post_meta($coupon->ID, 'wpdev_setup_fee_discount_type', true),
				'setup_fee_value'  => get_post_meta($coupon->ID, 'wpdev_setup_fee_discount_value', true),
				'limit_products'   => $limit_products,
				'allowed_products' => $allowed_products,
				'date_start'       => $coupon->date_create,
				'date_expiration'  => $wpdev_expiring_date,
			 ...
			 * Fix the type name.
			 */
			if (wpdev_get_isset($discount_code_data, 'type') === 'percent') {
			 ...
			 * Fix the type name.
			 */
			if (wpdev_get_isset($discount_code_data, 'setup_fee_type') === 'percent') {

				$discount_code_data['setup_fee_type'] = 'percentage';

			} // end if;

			$discount_code = wpdev_create_discount_code($discount_code_data);
	 ...
	 * Migrate Sites.
	 *
	 * The framework no longer carries site-domain logic. Sites example
	 * registers a migration callback via `wpdev_register_migration()`
	 * from its setup.php; this method is a thin dispatcher.
	 *
	 * @since 2.0.0
	 * @since 2.8.1 Domain logic moved to wpdev-examples/sites.
	 *
	 * @return void
	 */
	protected function _install_sites() {

		if ( function_exists( 'wpdev_run_migration_callback' ) ) {
			wpdev_run_migration_callback( 'sites' );
		} // end if;
	...
	protected function _install_site_templates() {
		 ...
		 */
		$skip_ids[] = wpdev_get_main_site_id();
		...
		if ($this->dry_run) {
			...
			$per_page = absint(wpdev_request('per_page', 10));
			$page     = absint(wpdev_request('page', 1));
		...
		if (is_array($saved_templates)) {
			...
			foreach ($saved_templates as $template_id) {
				...
				$site_template = wpdev_get_site($template_id);
				 ...
				 * Get Categories
				 */
				$categories_string = get_blog_option($site_template->get_id(), 'wpdev_categories', false);
				...
				if ($old_thumbnail_id) {
					...
					$attachment_id = \WPDevFramework\Helpers\Screenshot::save_image_from_url($image_url);
 ...
 protected function maybe_get_screenshot_url($site_id) {
	...
	protected function _install_domains() {
		 ...
		 * Load dependencies.
		 */
		wpdev_require_public_function( 'domain' );

		if ( ! function_exists( 'wpdev_create_domain' ) ) {
			return;
		}
		...
		foreach ($domains as $domain) {
			...
			$existing_domain = wpdev_get_domain_by_domain($domain->name);
			...
			$domain = wpdev_create_domain(array(
				'stage'          => 'done',
				'domain'         => $domain->name,
	...
	protected function _install_forms() {
		 ...
		 * Load dependencies.
		 */
		require_once dirname( __DIR__ ) . '/deprecated/deprecated.php';
		wpdev_require_public_function( 'legacy' );
		...
		/*
			...
			*/
		if (wpdev_get_checkout_form_by_slug('main-form')) {
		 ...
		 */
		wpdev_save_setting('default_login_page', $login_page_id);

		wpdev_save_setting('enable_custom_login_page', true);
	...
	protected function _install_emails() {
		...
		wpdev_require_public_function( 'webhook' );
		wpdev_require_public_function( 'broadcast' );

		if ( ! function_exists( 'wpdev_create_broadcast' ) ) {
			return;
		}
		...
		$broadcasts = $wpdb->get_results(
			"
				...
				FROM
					{$wpdb->base_prefix}posts
				WHERE
					post_type = 'wpdev_broadcast'
			"
		);

		foreach ($broadcasts as $broadcast) {

			$existing_broadcast = wpdev_get_broadcast_by('migrated_from_id', $broadcast->ID);
			...
			$old_type = get_post_meta($broadcast->ID, 'wpdev_type', true);

			$style = get_post_meta($broadcast->ID, 'wpdev_style', true);

			$new_type = $old_type === 'message' ? 'broadcast_notice' : 'broadcast_email';

			$customer_targets = (array) get_post_meta($broadcast->ID, 'wpdev_target_users', true);
			$product_targets  = (array) get_post_meta($broadcast->ID, 'wpdev_target_plans', true);

			$customer_targets = array_map(function($user_id) {

				$customer = wpdev_get_customer_by_user_id($user_id);
			...
			$product_targets = array_map(function($old_plan_id) {
				...
				$product = wpdev_call_if_function_exists( 'wpdev_get_product_by', null, 'migrated_from_id', $old_plan_id );
			...
			$broadcast = wpdev_create_broadcast(array(
	...
	protected function _install_webhooks() {
		...
		if ( ! function_exists( 'wpdev_create_webhook' ) ) {
			return;
		}
		...
		$webhooks = $wpdb->get_results(
			"
				...
				FROM
					{$wpdb->base_prefix}posts
				WHERE
					post_type = 'wpdev_webhook'
			"
		);

		foreach ($webhooks as $webhook) {

			$webhook = wpdev_create_webhook(array(
				'name'             => $webhook->post_title,
				'migrated_from_id' => $webhook->ID,
				'webhook_url'      => get_post_meta($webhook->ID, 'wpdev_url', true),
				'event'            => get_post_meta($webhook->ID, 'wpdev_event', true),
				'active'           => (bool) get_post_meta($webhook->ID, 'wpdev_active', true),
	...
	protected function _install_other() {
		...
		if ( ! class_exists( '\\WPDev\\Managers\\Domain_Manager' ) ) {
			return;
		}
