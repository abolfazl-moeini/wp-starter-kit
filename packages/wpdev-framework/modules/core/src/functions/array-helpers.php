...
use \WPDevFramework\Helpers\Arr;
...
function wpdev_array_merge_recursive_distinct(array &$array1, array &$array2, $should_sum = true) {
	...
	foreach ($array2 as $key => &$value) {
		...
		if (is_array($value) && isset($merged[$key]) && is_array($merged[$key])) {
			...
			$merged[$key] = wpdev_array_merge_recursive_distinct($merged[$key], $value, $should_sum);
	...
	foreach ($array1 as $key => $value) {
		...
		if (array_key_exists($key, $array2)) {
			...
			if (is_array($value)) {
				...
				$array_recursive_diff = wpdev_array_recursive_diff($value, $array2[$key], $to_keep);
