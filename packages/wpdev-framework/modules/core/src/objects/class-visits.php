	...
	public function __construct($site_id)
 {
	...
	public function get_visits($start_date = false, $end_date = false) {
		...
		if (!$start_date) {
			...
			$start_date = wpdev_get_current_time('mysql', true);

		} // end if;

		if (!$end_date) {

			$end_date = wpdev_get_current_time('mysql', true);

		} // end if;

		$query = $wpdb->prepare("
      SELECT meta_value as count, str_to_date(meta_key, 'wpdev_visits_%%Y%%m%%d') as day, blog_id as site_id
      FROM {$wpdb->base_prefix}blogmeta 
      WHERE blog_id = %d
    ", $this->site_id);

		$query .= $wpdb->prepare(" AND str_to_date(meta_key, 'wpdev_visits_%%Y%%m%%d') BETWEEN %s AND %s", gmdate('Y-m-d', strtotime($start_date)), gmdate('Y-m-d', strtotime($end_date)));
    ...
    ";
		...
		$sub_query .= $wpdb->prepare(" WHERE str_to_date(meta_key, 'wpdev_visits_%%Y%%m%%d') BETWEEN %s AND %s", gmdate('Y-m-d', strtotime($start_date)), gmdate('Y-m-d', strtotime($end_date)));
