...
class Billing_Address {
	...
	public function __isset($name) {
		...
		return wpdev_get_isset($this->attributes, $name, '');
	...
	public function __get($name) {
		...
		$value = wpdev_get_isset($this->attributes, $name, '');

		return apply_filters("wpdev_billing_address_get_{$name}", $value, $this);
	...
	public function __set($name, $value) {
		...
		$value = apply_filters("wpdev_billing_address_set_{$name}", $value, $this);
	...
	protected function validation_rules(): array {
		...
		$keys = array_keys(array_filter($fields, fn($item) => wpdev_get_isset($item, 'validation_rules')));
	 ...
	 * @since 2.0.0
	 * @param bool                            $zip_only If we only need zip and country.
	 * @param \WPDevFramework\Models\Checkout_Form $checkout_form The current checkout form.
	 * @return array
	 */
	public static function fields($zip_only = false, $checkout_form = false) {
		...
		$countries = wpdev_get_countries_as_options();
		...
		$fields['company_name'] = array(
			'type'                => 'text',
			'title'               => __('Company Name', 'wpdev'),
			'default_placeholder' => __('E.g. Google (optional)', 'wpdev'),
			'wrapper_classes'     => 'sm:wpdev-col-span-1',
		);

		$fields['billing_email'] = array(
			'type'                => 'text',
			'title'               => __('Billing Email', 'wpdev'),
			'default_placeholder' => __('E.g. john@company.com', 'wpdev'),
			'wrapper_classes'     => 'sm:wpdev-col-span-1',
			'required'            => true,
		);
		...
		$fields['billing_address_line_1'] = array(
			'type'                => 'text',
			'title'               => __('Address Line 1', 'wpdev'),
			'default_placeholder' => __('E.g. 555 1st Avenue', 'wpdev'),
			'wrapper_classes'     => 'wpdev-col-span-2',
			'required'            => true,
		);

		$fields['billing_address_line_2'] = array(
			'type'                => 'text',
			'title'               => __('Address Line 2', 'wpdev'),
			'default_placeholder' => __('E.g. Apartment 10a', 'wpdev'),
			'wrapper_classes'     => 'wpdev-col-span-2',
		);

		$fields['billing_country'] = array(
			'type'                => 'select',
			'title'               => __('Country', 'wpdev'),
			'default_placeholder' => __('E.g. US', 'wpdev'),
			'wrapper_classes'     => 'sm:wpdev-col-span-1',
			'value'               => ' ',
			'options'             => $countries,
			'required'            => true,
		...
		$fields['billing_state'] = array(
			'type'                => 'text',
			'title'               => __('State / Province', 'wpdev'),
			'default_placeholder' => __('E.g. NY', 'wpdev'),
			'wrapper_classes'     => 'sm:wpdev-col-span-1',
		);

		$fields['billing_city'] = array(
			'type'                => 'text',
			'title'               => __('City / Town', 'wpdev'),
			'default_placeholder' => __('E.g. New York City', 'wpdev'),
			'wrapper_classes'     => 'sm:wpdev-col-span-1',
		);

		$fields['billing_zip_code'] = array(
			'type'                => 'text',
			'title'               => __('ZIP / Postal Code', 'wpdev'),
			'default_placeholder' => __('E.g. 10009', 'wpdev'),
			'wrapper_classes'     => 'sm:wpdev-col-span-1',
			'required'            => true,
		);

		$fields = wpdev_set_order_from_index($fields); // Adds missing order attributes
	   ...
	   * @param bool  $zip_only If we only need zip and country.
		 * @return array
		 */
		$fields = apply_filters('wpdev_billing_address_fields', $fields, $zip_only);
	...
	public static function fields_for_rest($zip_only = false) {
		...
		foreach (self::fields($zip_only) as $field_key => $field) {
			...
			$options = wpdev_get_isset($field, 'options', false);

			$enum = is_callable($options) ? call_user_func($options) : false;

			$fields_for_rest[$field_key] = array(
				'description' => wpdev_get_isset($field, 'title', false) . '. ' . wpdev_get_isset($field, 'default_placeholder', false),
				'type'        => 'string',
				'required'    => wpdev_get_isset($field, 'required', false),
			);
