 ...
 *
 * Handles product compatibility back-ports to WPDev 1.X builds.
 *
 * @package WPDev
...
class Product_Compat {
	...
	public function init() {
		...
		add_filter('wpdev_product_options_sections', array($this, 'add_legacy_section'), 100, 2);
	...
	public function check_update_plan($null, $object_id, $meta_key, $meta_value, $prev_value) {
		...
		$migrated_product = wpdev_get_product($object_id);
		 ...
		 */
		$meta_key = str_replace('wpdev_', '', $meta_key);

		/*
		 * Save using the new meta table.
		 */
		$migrated_product->update_meta('wpdev_' . $meta_key, maybe_serialize($meta_value));
	 ...
	 * @param \WPDevFramework\Models\Product $object The model being edited.
	 * @return array
	 */
	public function add_legacy_section($sections, $object) {

		$sections['legacy_options_core'] = array(
			'title'  => __('Legacy Options', 'wpdev'),
			'desc'   => __('Options used by old 1.X versions. ', 'wpdev'),
			'icon'   => 'dashicons-wpdev-spreadsheet',
			'state'  => array(
			...
			'fields' => array(
				'legacy_options' => array(
					...
					'title'     => __('Toggle Legacy Options', 'wpdev'),
					'desc'      => __('Toggle this option to edit legacy options.', 'wpdev'),
					'html_attr' => array(
				...
				'featured_plan'  => array(
					...
					'title'             => __('Featured Plan', 'wpdev'),
					'desc'              => __('Toggle this option to mark this product as featured on the legacy pricing tables.', 'wpdev'),
				...
				'feature_list'   => array(
					'type'              => 'textarea',
					'title'             => __('Features List', 'wpdev'),
					'placeholder'       => __('E.g. Feature 1', 'wpdev') . PHP_EOL . __('Feature 2', 'wpdev'),
					'desc'              => __('Add a feature per line. These will be shown on the pricing tables.', 'wpdev'),
