<?php
/**
 * Handles the Admin Notices added by WPDev.
 *
 * @package WPDev
...
class Admin_Notices {
	...
	/**
	 * Holds the notices added by WPDev.
	 *
	 * @since 2.0.0
	 * @var array
	 */
	...
	public function init() {
		...
		wpdev_register_ajax_handler( 'wpdev_dismiss_admin_notice', array( $this, 'ajax_dismiss_admin_notices' ) );
	...
	public function get_dismissed_notices() {
		...
		$dismissed = get_user_meta(get_current_user_id(), 'wpdev_dismissed_admin_notices', true);
	...
	public function get_notices($panel = 'admin', $filter = true) {
		...
		/**
		 * Allow developers to filter admin notices added by WPDev.
		 *
		 * @since 2.0.0
		 *
	   ...
	   * @return array
		 */
		return apply_filters('wpdev_admin_notices', $notices, $this->notices, $panel, $filter, $dismissed_messages);
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_script('wpdev-admin-notices', wpdev_get_module_asset_url('core', 'admin-notices.js', 'js'), array('jquery'), wpdev_get_version());
	...
	public function display_notices() {
		...
		wpdev_get_template('admin-notices', array(
			'notices' => $notices,
			'nonce'   => wp_create_nonce('wpdev-dismiss-admin-notice'),
		));
	...
	public function ajax_dismiss_admin_notices() {
		...
		if (!wp_verify_nonce($_POST['nonce'], 'wpdev-dismiss-admin-notice')) {
		...
		if (!in_array($_POST['notice_id'], $dismissed, true)) {
			...
			update_user_meta(get_current_user_id(), 'wpdev_dismissed_admin_notices', $dismissed);
