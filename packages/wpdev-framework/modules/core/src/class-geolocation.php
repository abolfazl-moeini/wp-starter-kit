...
class Geolocation {
    ...
    public static function init() {
        if ( self::supports_geolite2() ) {
            ...
            if ( self::is_geolocation_enabled( get_option( 'wpdev_default_customer_address' ) ) || apply_filters( 'wpdev_geolocation_update_database_periodically', false ) ) {
                add_action( 'wpdev_geoip_updater', array( __CLASS__, 'update_database' ) );
            } // end if;

            // Trigger database update when settings are changed to enable geolocation.
            add_filter( 'pre_update_option_wpdev_default_customer_address', array( __CLASS__, 'maybe_update_database' ), 10, 2 );
        } else {
            add_filter( 'pre_option_wpdev_default_customer_address', array( __CLASS__, 'disable_geolocation_on_legacy_php' ) );
        } // end if;
    } // end init;
    ...
    public static function get_external_ip_address() {
        ...
        if ( false === $external_ip_address ) {
            $external_ip_address     = '0.0.0.0';
            $ip_lookup_services      = apply_filters( 'wpdev_geolocation_ip_lookup_apis', self::$ip_lookup_apis );
            ...
            foreach ( $ip_lookup_services_keys as $service_name ) {
                ...
                if ( !is_wp_error( $response ) && rest_is_ip_address( $response['body'] ) ) {
                    $external_ip_address = apply_filters( 'wpdev_geolocation_ip_lookup_api_response', ( $response['body'] ), $service_name );
                    break;
                } // end if;
            } // end foreach;
    ...
    public static function geolocate_ip($ip_address = '', $fallback = false, $api_fallback = true ) {
        ...
        $country_code = apply_filters( 'wpdev_geolocate_ip', false, $ip_address, $fallback, $api_fallback );
    ...
    public static function get_local_database_path($deprecated = '2' ) {
        return apply_filters( 'wpdev_geolocation_local_database_path', WP_CONTENT_DIR . '/uploads/GeoLite2-Country.mmdb', $deprecated );
    ...
    public static function update_database() {
        ...
        if ( !is_wp_error( $tmp_database_path ) ) {
            ...
            try {
                ...
                wp_clear_scheduled_hook( 'wpdev_geoip_updater' );
                wp_schedule_event( strtotime( 'first tuesday of next month' ), 'monthly', 'wpdev_geoip_updater' );
            } // end try;
     ...
     *
     * Geolocation APIs can be added through the use of the wpdev_geolocation_geoip_apis filter.
    ...
    private static function geolocate_via_api($ip_address ) {
        ...
        if ( false === $country_code ) {
            $geoip_services = apply_filters( 'wpdev_geolocation_geoip_apis', self::$geoip_apis );
                ...
                if ( !is_wp_error( $response ) && $response['body'] ) {
                    switch ( $service_name ) {
                        ...
                        case 'ip-api.com':
                            ...
                            break;
                        default:
                            $country_code = apply_filters( 'wpdev_geolocation_geoip_response_' . $service_name, '', $response['body'] );
                            break;
                    } // end switch;
