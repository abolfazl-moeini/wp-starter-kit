...
class Light_Ajax {
	...
	public function __construct() {
		...
		if (isset($_REQUEST['wpdev-ajax'])) {

			$action = $this->get_when_to_run();

			wpdev_x_header("X-wpdev-Ajax-When: $action");
	...
	protected function should_skip_referer_check(): bool {
		...
		$allowed_actions = apply_filters('wpdev_light_ajax_should_skip_referer_check', array(
			 ...
			 */
			'wpdev_render_field_template',
			'wpdev_create_order',
			'wpdev_validate_form',

		));

		return in_array(wpdev_request('action', 'no-action'), $allowed_actions, true);
	...
	public function get_when_to_run() {
		 ...
		 */
		$allowed_list = apply_filters('wpdev_light_ajax_allowed_hooks', array(
			'plugins_loaded',
			'setup_theme',
			'after_setup_theme',
			'init',
		...
		$action = isset($_REQUEST['wpdev-when']) ? base64_decode((string) $_REQUEST['wpdev-when']) : 'plugins_loaded';
	...
	public function process_light_ajax() {
		 ...
		 * as an API endpoint.
		 */
		$this->should_skip_referer_check() === false && check_ajax_referer('wpdev-ajax-nonce', 'r');
		 ...
		 * @since 2.0.0
		 */
		do_action('wpdev_before_light_ajax');

		/** Allow for cross-domain requests (from the front end). */
		send_origin_headers();

		header('Content-Type: text/html; charset=' . get_blog_option(wpdev_get_main_site_id(), 'blog_charset'));
		...
		if (is_user_logged_in()) {
			...
			do_action('wpdev_ajax_' . $action); // phpcs:ignore

		} else {

			do_action('wpdev_ajax_nopriv_' . $action); // phpcs:ignore
