...
abstract class Country {
	...
	public function __get($attribute) {
		...
		return wpdev_get_isset($this->attributes, $attribute, null);
	...
	public function get_states() {
		 ...
		 * @param WPDevFramework\Country\Country $current_country Instance of the current class.
		 * @return array The filtered list of states.
		 */
		return apply_filters('wpdev_country_get_states', $states, $this->country_code, $this);
	...
	public function get_states_as_options($placeholder = '') {
		...
		if ($placeholder !== false && $options) {
			...
			$placeholder_option[''] = $placeholder !== '' ? $placeholder : sprintf(__('Select your %s', 'wpdev'), $division_name);
	...
	public function get_cities($state_code = '') {
		...
		$repository_file = wpdev_path( "modules/core/src/country/{$this->country_code}/{$state_code}.php" );
		 ...
		 * @return array The filtered list of states.
		 */
		return apply_filters('wpdev_country_get_cities', $cities, $this->country_code, $state_code, $this);
	...
	public function get_cities_as_options($state_code = '', $placeholder = '') {
		...
		if ($placeholder !== false && $options) {
			...
			$placeholder_option[''] = $placeholder !== '' ? $placeholder : __('Select your city', 'wpdev');
	...
	public function get_municipality_name($state_code = null, $ucwords = false) {
		...
		return apply_filters('wpdev_country_get_municipality_name', $name, $this->country_code, $state_code, $ucwords, $this);
	...
	public function get_administrative_division_name($state_code = null, $ucwords = false) {
		...
		$denominations = array(
			'province'             => __('province', 'wpdev'),
			'state'                => __('state', 'wpdev'),
			'territory'            => __('territory', 'wpdev'),
			'region'               => __('region', 'wpdev'),
			'department'           => __('department', 'wpdev'),
			'district'             => __('district', 'wpdev'),
			'prefecture'           => __('prefecture', 'wpdev'),
			'autonomous_community' => __('autonomous community', 'wpdev'),
			'parish'               => __('parish', 'wpdev'),
			'county'               => __('county', 'wpdev'),
			'division'             => __('division', 'wpdev'),
			'unknown'              => __('state / province', 'wpdev'),
		);

		$name = wpdev_get_isset($denominations, $this->state_type, $denominations['unknown']);
		 ...
		 */
		return apply_filters('wpdev_country_get_administrative_division_name', $name, $this->country_code, $state_code, $ucwords, $this);
