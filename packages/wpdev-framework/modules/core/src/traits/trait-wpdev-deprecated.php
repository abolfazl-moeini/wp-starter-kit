<?php
/**
 * A trait to be included in entities to WPDev Class depecrated methods.
 *
 * @package WPDev
 * @subpackage Apis
 * @since 2.0.0
...
/**
 * WPDev_Deprecated trait.
 */
trait WPDev_Deprecated {

	/**
	 * Deprecated: WPDev->slugfy().
	 *
	 * @since 2.0.0
	...
	public function slugfy($term) {
		...
		_deprecated_function(__METHOD__, '2.0.0', 'wpdev_slugify($term)');
	...
	/**
	 * Deprecated: WPDev->add_page_to_branding()
	 *
	 * @since 2.0.0
	 * @return void
	 */
	...
	public function render($view, $vars = false) {
		...
		_deprecated_function(__METHOD__, '2.0.0', 'wpdev_get_template()');

		wpdev_get_template($view, $vars);
	...
	public function path($dir) {
		...
		_deprecated_function(__METHOD__, '2.0.0', 'wpdev_path()');
	...
	public function add_message($message, $type = 'success', $network = false) {
		...
		_deprecated_function(__METHOD__, '2.0.0', 'WPDev()->notices->add()');

		$panel = $network ? 'network-admin' : 'admin';

		$wpdev = wpdev();

		if (isset($wpdev->notices) && $wpdev->notices) {

			$wpdev->notices->add($message, $type, $panel);
	...
	public function check_before_run() {
		...
		_deprecated_function(__METHOD__, '2.0.0', 'WPDev()->is_loaded()');

		return wpdev()->is_loaded();
	...
	public function enqueue_select2() {
		...
		wp_enqueue_style('wpdev-select2css', 'https://cdnjs.cloudflare.com/ajax/libs/select2/3.4.8/select2.css', false, '1.0', 'all');

		wp_enqueue_script('wpdev-select2', 'https://cdnjs.cloudflare.com/ajax/libs/select2/3.4.8/select2.js', array('jquery'), '1.0', true);

	} // end enqueue_select2;

} // end trait WPDev_Deprecated;
