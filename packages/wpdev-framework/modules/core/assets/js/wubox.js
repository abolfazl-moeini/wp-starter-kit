...
const formSubmit = (form) => async (event) => {
  ...
  const blocked_form = wpdev_block_ui(form);
  if (window["wpdev_" + form.getAttribute("id") + "_errors"]) {
    window["wpdev_" + form.getAttribute("id") + "_errors"].errors = [];
  }
  const submitButton = event.submitter && event.submitter.value ? event.submitter.value : "submit";
  const formData = new FormData(form);
  formData.append("submit", submitButton);
  let response = null;
  try {
    const raw = await fetch(form.getAttribute("action"), {
      method: "POST",
      body: formData,
      headers: {
        "X-Requested-With": "XMLHttpRequest"
  ...
  if (!response.success) {
    ...
    if (window["wpdev_" + formId + "_errors"]) {
      window["wpdev_" + formId + "_errors"].errors = response.data;
    }
    const formApp = document.querySelector('[data-wpdev-app="' + formId + '_errors"]');
  ...
  if (typeof response.data.redirect_url === "string" && response.data.redirect_url) {
...
const setBodyListeners = (domChunk) => {
  ...
  document.body.addEventListener("wubox:load", () => {
    const form = document.querySelector("#WUB_ajaxContent .wpdev_form");
    if (!form) {
      return;
    }
    initForm(form);
