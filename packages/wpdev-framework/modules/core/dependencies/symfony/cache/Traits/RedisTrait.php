 ...
 */
namespace WPDev\Dependencies\Symfony\Component\Cache\Traits;

use WPDev\Dependencies\Predis\Command\Redis\UNLINK;
use WPDev\Dependencies\Predis\Connection\Aggregate\ClusterInterface;
use WPDev\Dependencies\Predis\Connection\Aggregate\RedisCluster;
use WPDev\Dependencies\Predis\Connection\Aggregate\ReplicationInterface;
use WPDev\Dependencies\Predis\Response\ErrorInterface;
use WPDev\Dependencies\Predis\Response\Status;
use WPDev\Dependencies\Symfony\Component\Cache\Exception\CacheException;
use WPDev\Dependencies\Symfony\Component\Cache\Exception\InvalidArgumentException;
use WPDev\Dependencies\Symfony\Component\Cache\Marshaller\DefaultMarshaller;
use WPDev\Dependencies\Symfony\Component\Cache\Marshaller\MarshallerInterface;
/**
...
trait RedisTrait
{
    ...
    private function init($redis, string $namespace, int $defaultLifetime, ?MarshallerInterface $marshaller)
    {
        ...
        if (!$redis instanceof \Redis && !$redis instanceof \RedisArray && !$redis instanceof \RedisCluster && !$redis instanceof \WPDev\Dependencies\Predis\ClientInterface && !$redis instanceof RedisProxy && !$redis instanceof RedisClusterProxy) {
        ...
        if ($redis instanceof \WPDev\Dependencies\Predis\ClientInterface && $redis->getOptions()->exceptions) {
    ...
    public static function createConnection(string $dsn, array $options = [])
    {
        ...
        if (!\extension_loaded('redis') && !\class_exists(\WPDev\Dependencies\Predis\Client::class)) {
        ...
        if (isset($params['redis_sentinel']) && !\class_exists(\WPDev\Dependencies\Predis\Client::class) && !\class_exists(\RedisSentinel::class)) {
        ...
        if (null === $params['class'] && \extension_loaded('redis')) {
            ...
            $class = $params['class'] ?? \WPDev\Dependencies\Predis\Client::class;
            if (isset($params['redis_sentinel']) && !\is_a($class, \WPDev\Dependencies\Predis\Client::class, \true) && !\class_exists(\RedisSentinel::class)) {
        ...
        } elseif (\is_a($class, \WPDev\Dependencies\Predis\ClientInterface::class, \true)) {
    ...
    protected function doFetch(array $ids)
    {
        ...
        $result = [];
        if ($this->redis instanceof \WPDev\Dependencies\Predis\ClientInterface && $this->redis->getConnection() instanceof ClusterInterface) {
    ...
    {
        if ($this->redis instanceof \WPDev\Dependencies\Predis\ClientInterface) {
        ...
        $hosts = $this->getHosts();
        $host = \reset($hosts);
        if ($host instanceof \WPDev\Dependencies\Predis\Client && $host->getConnection() instanceof ReplicationInterface) {
        ...
        foreach ($hosts as $host) {
            ...
            if (!$host instanceof \WPDev\Dependencies\Predis\ClientInterface) {
            ...
            if (!\version_compare($info['redis_version'], '2.8', '>=')) {
                ...
                $args = $this->redis instanceof \WPDev\Dependencies\Predis\ClientInterface ? [0, $pattern] : [[$pattern], 0];
            ...
            $cursor = null;
            do {
                $keys = $host instanceof \WPDev\Dependencies\Predis\ClientInterface ? $host->scan($cursor, 'MATCH', $pattern, 'COUNT', 1000) : $host->scan($cursor, $pattern, 1000);
    ...
    private function pipeline(\Closure $generator, object $redis = null) : \Generator
    {
        ...
        if ($redis instanceof RedisClusterProxy || $redis instanceof \RedisCluster || $redis instanceof \WPDev\Dependencies\Predis\ClientInterface && $redis->getConnection() instanceof RedisCluster) {
            ...
            foreach ($generator() as $command => $args) {
                ...
                $ids[] = 'eval' === $command ? $redis instanceof \WPDev\Dependencies\Predis\ClientInterface ? $args[2] : $args[1][0] : $args[0];
            }
        } elseif ($redis instanceof \WPDev\Dependencies\Predis\ClientInterface) {
        ...
        if (!$redis instanceof \WPDev\Dependencies\Predis\ClientInterface && 'eval' === $command && $redis->getLastError()) {
        ...
        if ($this->redis instanceof \WPDev\Dependencies\Predis\ClientInterface) {
            ...
            if ($connection instanceof ClusterInterface && $connection instanceof \Traversable) {
                ...
                foreach ($connection as $c) {
                    $hosts[] = new \WPDev\Dependencies\Predis\Client($c);
                }
            }
