...
use WPDev\Dependencies\Predis\Connection\Aggregate\PredisCluster;
...
use WPDev\Dependencies\Symfony\Component\Cache\CacheItem;
use WPDev\Dependencies\Symfony\Component\Cache\Exception\InvalidArgumentException;
use WPDev\Dependencies\Symfony\Component\Cache\Exception\LogicException;
use WPDev\Dependencies\Symfony\Component\Cache\Marshaller\DeflateMarshaller;
use WPDev\Dependencies\Symfony\Component\Cache\Marshaller\MarshallerInterface;
use WPDev\Dependencies\Symfony\Component\Cache\Marshaller\TagAwareMarshaller;
use WPDev\Dependencies\Symfony\Component\Cache\Traits\RedisClusterProxy;
use WPDev\Dependencies\Symfony\Component\Cache\Traits\RedisProxy;
use WPDev\Dependencies\Symfony\Component\Cache\Traits\RedisTrait;
/**
...
class RedisTagAwareAdapter extends AbstractTagAwareAdapter
{
    ...
    {
        if ($redis instanceof \WPDev\Dependencies\Predis\ClientInterface && $redis->getConnection() instanceof ClusterInterface && !$redis->getConnection() instanceof PredisCluster) {
...
EOLUA;
        $results = $this->pipeline(function () use($ids, $lua) {
            foreach ($ids as $id) {
                (yield 'eval' => $this->redis instanceof \WPDev\Dependencies\Predis\ClientInterface ? [$lua, 1, $id] : [$lua, [$id], 1]);
            }
        });
        foreach ($results as $id => $result) {
...
EOLUA;
        $results = $this->pipeline(function () use($tagIds, $lua) {
            ...
            foreach ($tagIds as $id) {
                (yield 'eval' => $this->redis instanceof \WPDev\Dependencies\Predis\ClientInterface ? [$lua, 1, $id, $prefix] : [$lua, [$id, $prefix], 1]);
            }
        });
        $lua = <<<'EOLUA'
            redis.replicate_commands()
