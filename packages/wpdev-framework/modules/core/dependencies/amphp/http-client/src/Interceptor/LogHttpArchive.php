...
use WPDev\Dependencies\Amp\File;
...
use WPDev\Dependencies\Amp\Http\Client\EventListener;
use WPDev\Dependencies\Amp\Http\Client\EventListener\RecordHarAttributes;
    ...
    private function writeLog(Response $response) : Promise
    {
        return call(function () use($response) {
            ...
            try {
                ...
                if ($firstEntry) {
                    $this->fileHandle = (yield \function_exists('WPDev\\Dependencies\\Amp\\File\\openFile') ? File\openFile($this->filePath, 'w') : File\open($this->filePath, 'w'));
