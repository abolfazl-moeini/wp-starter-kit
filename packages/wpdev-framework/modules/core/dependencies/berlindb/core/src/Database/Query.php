...
class Query extends Base
{
     ...
     * @var   string
     */
    protected $table_schema = 'WPDev\\Dependencies\\BerlinDB\\Database\\Schema';
    /** Item ******************************************************************/
    /**
     ...
     * @var   mixed
     */
    protected $item_shape = 'WPDev\\Dependencies\\BerlinDB\\Database\\Row';
    /** Cache *****************************************************************/
    /**
    ...
    private function get_table_name()
    {
        ...
        if (!empty($db) && isset($db->{$this->table_name})) {
    ...
    {
        $type = $this->apply_prefix($this->item_name);
        if (str_starts_with($type, 'wu_')) {
            return 'wpdev_' . substr($type, 3);
        }
        return $type;
    }
    /** Cache *****************************************************************/
    ...
    private function prime_item_caches($item_ids = array(), $force = \false)
    {
        ...
        if (!empty($this->query_vars['update_meta_cache'])) {
            ...
            if (str_starts_with($singular, 'wu_')) {
                $singular = 'wpdev_' . substr($singular, 3);
            }
            // sic
