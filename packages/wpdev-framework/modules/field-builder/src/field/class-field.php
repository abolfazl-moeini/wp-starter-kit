...
class Field implements \JsonSerializable {
	...
	public function get_compat_template_name() {
		...
		if (array_key_exists($this->type, $aliases)) {
			...
			if (array_key_exists($this->type, $deprecated)) {
				...
				_doing_it_wrong('wpdev_add_field', sprintf(__('The field type "%1$s" is no longer supported, use "%2$s" instead.'), $this->type, $new_type_name), '2.0.0');
	...
	public function __get($att) {
		...
		$allow_callable_prefix = is_string($attr) && strncmp($attr, 'wpdev_get_', strlen('wpdev_get_')) === 0 && is_callable($attr);
		...
		if ($att === 'wrapper_classes' && isset($this->atts['wrapper_html_attr']['v-show'])) {
			...
			$this->atts['wrapper_classes'] = $this->atts['wrapper_classes'] . ' wpdev-requires-other';

		} // end if;

		if ($att === 'type' && $this->atts[$att] === 'submit') {

			$this->atts['wrapper_classes'] = $this->atts['wrapper_classes'] . ' wpdev-submit-field';

		} // end if;

		if ($att === 'type' && $this->atts[$att] === 'tab-select') {

			$this->atts['wrapper_classes'] = $this->atts['wrapper_classes'] . ' wpdev-tab-field';
	...
	protected function sanitization_rules() {
		...
		$rules = array(
			...
			'checkbox'       => 'wpdev_string_to_bool',
			'multi_checkbox' => false,
			'select2'        => false,
			'multiselect'    => false,
		);

		return apply_filters('wpdev_settings_fields_sanitization_rules', $rules);
	 ...
	 * @return WPDevFramework\UI\Field
	 */
	...
	public function sanitize() {
		...
		$type = isset( $this->atts['type'] ) ? (string) $this->atts['type'] : 'text';

		if ( class_exists( 'WPDev\\Modules\\FieldBuilder\\Field_Type_Registry' )
			&& null !== \WPDevFramework\Modules\FieldBuilder\Field_Type_Registry::get( $type ) ) {

			$field_def = is_array( $this->atts ) ? $this->atts : array();

			if ( isset( $this->min ) ) {
				$field_def['min'] = $this->min;
			}

			if ( isset( $this->max ) ) {
				$field_def['max'] = $this->max;
			}

			$this->value = \WPDevFramework\Modules\FieldBuilder\Field_Type_Registry::sanitize(
				$type,
				$this->value,
				$field_def
			);
		...
		$sanitize_method = isset( $rules[ $type ] ) ? $rules[ $type ] : $rules['text'];
		...
		/**
		 * Legacy validation hook for unregistered field types.
		 *
		 * @since 2.6.0
		 *
		 ...
		 * @param array $atts  Field attributes.
		 */
		$this->value = apply_filters( "wpdev_field_validate_{$type}", $this->value, $this->atts );
 ...
 protected function validate_number_field($value) {
	...
	public function get_html_attributes() {
		...
		return wpdev_array_to_html_attrs($attributes);
