...
describe('buildAssets --dry-run mode (end-to-end with real config)', () => {
    ...
    test('process.exit(1) is called when build.config.json is missing', async () => {
        ...
        try {
            const sut = await import('../../build/build-assets.js');
            await sut.buildAssets();
            // The catch block should call process.exit(1) on read failure
...
describe('validateConfig with mock config', () => {
    ...
    test('throws when globalMappings value is not a string', () => {
        ...
        expect(() => validateConfig(config)).toThrow('globalMappings["tabulator-tables"] must be a string');
    });

    test('throws when globalMappings is not an object', () => {
        const config = { assetMappings: [], globalMappings: [] };
        expect(() => validateConfig(config)).toThrow('globalMappings must be an object');
    });
    ...
    test('throws when styleEntryPoints is not an array', () => {
        const config = { assetMappings: [], styleEntryPoints: 'assets/stylesheets/style.css' };
        expect(() => validateConfig(config)).toThrow('styleEntryPoints must be an array');
    });

    test('throws when styleEntryPoint item is not a string', () => {
        const config = { assetMappings: [], styleEntryPoints: [123] };
        expect(() => validateConfig(config)).toThrow('Each styleEntryPoint must be a string');
    });
});
