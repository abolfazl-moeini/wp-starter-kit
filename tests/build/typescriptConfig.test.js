...
function globListIncludes(include, pattern) {
  ...
  return include.some(
    (entry) => typeof entry === "string" && entry === pattern,
  );
}
...
describe("TypeScript configuration contract (root)", () => {
  ...
  beforeAll(() => {
    ...
    jestConfigSource = existsSync(jestConfigPath)
  ...
  test("tsconfig.json includes assets/**/*.ts and packages/**/*.ts", () => {
    ...
    const hasPackagesGlob = globListIncludes(
      tsconfig.include,
      "packages/**/*.ts",
    );
    const hasPackagesFiles = filesListIncludes(
      tsconfig.files,
      "packages/**/*.ts",
    );
  ...
  test("jest.config.mjs has a transform entry covering .ts/.tsx files", () => {
    ...
    const hasPresetTypescript = jestConfigSource.includes(
