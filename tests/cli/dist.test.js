 ...
 *   - `bin.wpsk` maps to a real file that is a Node ESM entry (shebang).
 ...
 *   - The bin file resolves to ESM source so `node bin/wpsk.js` and
 *     `npx wpsk` both work.
 *
 ...
 *     - "bin.wpsk": "bin/wpsk.js"
 *     - "files": [ "bin", "src" ]
 *     - "engines.node": ">=18"
 *   packages/cli/bin/wpsk.js
...
const BIN_FILE = join(PKG_ROOT, "bin", "wpsk.js");
...
describe("@wpsk/cli distribution — bin executable contract", () => {
  test("package.json declares bin.wpsk → bin/wpsk.js", () => {
    const pkg = loadPkg();
    expect(pkg.bin).toBeDefined();
    expect(pkg.bin.wpsk).toBe("bin/wpsk.js");
  });

  test("bin/wpsk.js exists on disk and is a file", () => {
  ...
  test("bin/wpsk.js is executable on disk (mode includes owner-x bit)", () => {
    const st = statSync(BIN_FILE);
  ...
  test("bin/wpsk.js delegates to src/main.js (ESM import, not require)", () => {
